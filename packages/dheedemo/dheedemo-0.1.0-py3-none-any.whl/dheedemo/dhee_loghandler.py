...
class DheeLoggerHandler(logging.Handler):

    def __init__(self, **logparams) -> None:
        super().__init__()
        self.log_integration_list = logparams.get('log_integration_list')
        if logparams.get('appContext') is not None:
            self.appContext = logparams.get('appContext')
    ...
    def post_logs_to_influxdb(self, record: logging.LogRecord):
        ...
        if hasattr(self,'appContext') and self.appContext is not None:
    ...
    def post_logs_to_cloudwatch(self, record: logging.LogRecord):
        ...
        """
        if hasattr(self,'appContext') and self.appContext is not None:
            glue_logger = self.appContext.get_logger()
            message = self.format(record)
