...
class Element(Variable):
    ...
    def __init__(self, name: str, belong_to, uuid: tp.Optional[str] = None):
        """
        Element object.
        ...
        Examples:
            ...
            ````
        """
        if isinstance(belong_to, tuple):
            self._parent: tp.Union[Range, Variable] = Range(belong_to[0], belong_to[1])
        elif isinstance(belong_to, _expression.Expression):
            if belong_to.is_operatable():
                # operatable means belong_to is scalar.
                self._parent = Range(start=0, last=belong_to)
            else:
                # belong_to is not scalar. ex. i in V
                self._parent = belong_to  # type: ignore
        elif isinstance(belong_to, (int, float)):
            # belong_to is range. ex. i in V
            self._parent = Range(start=0, last=int(belong_to))
        elif isinstance(belong_to, Range):
            self._parent = belong_to
        else:
            raise TypeError(
