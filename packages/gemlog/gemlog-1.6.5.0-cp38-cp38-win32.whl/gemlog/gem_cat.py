...
def gem_cat(input_dir, output_dir, ext="", cat_all=False):
    ...
    out_file = (
        []
    )  ## note that out_file being empty acts as a flag saying that no files have been processed yet
    try:
        SN = pd.read_csv(
            gem_files[0], delimiter=",", skiprows=4, nrows=1, dtype="str", names=["s", "SN"]
        ).SN[0]
    except:
