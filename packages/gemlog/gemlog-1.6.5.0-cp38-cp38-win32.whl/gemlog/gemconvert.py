...
def convert_single_SN(arg_list):
    ...
    try:
        ...
        gemlog.convert(
            inputdir,
            SN=SN,
            convertedpath=outputdir,
            output_format=output_format,
            file_length_hour=output_length,
        )
        print(f"{SN} done")
...
def print_call():
    print(
        "gemconvert -i <inputdir> -s <serialnumbers> -x <exclude_serialnumbers> -o <outputdir> -f <format> -l <filelength_hours> -p <number_of_processes>"
    )
    ...
    print(
        "-p --parallel: number of processes to run in parallel (limited by your computer); default 1."
    )
...
def main(argv=None):
    ...
    logging.basicConfig(
        level=debug_level,
        filename="gemconvert_logfile.txt",
        filemode="a+",
    ...
    try:
        ...
        print(
            "Problem opening input data folder "
            + inputdir
    ...
    if len(SN_list) == 0:  # if user does not provide SN_list, take unique SNs in order
        SN_list = unique(
            [find_SN(file_list[i]) for i in range(len(file_list))]
    ...
    if not test:
        ...
        logging.info(
            f"Dependencies: Python {platform.python_version()}, obspy {gemlog.core.obspy.__version__}, pandas {gemlog.core.pd.__version__}, scipy {gemlog.core.scipy.__version__}, numpy {gemlog.core.np.__version__}"
        )
        logging.info(
            f"platform.uname(): {p.system}, {p.release}, {p.version}, {p.machine}, {p.processor}"
        )
        ...
        logging.info(
            f'format="{output_format}", length_hours={output_length}, test={test}, parallel={num_processes}'
        )
