        ...
        for i in range(0, len(dataframe)):
            ...
            for j in range(0, len(dataframe.columns)):
                if type(dataframe.iloc[i, j]) is not str:
                    ...
                    self.cell(
                        cell_width, cell_height, f"{np.round(dataframe.iloc[i,j],3)}", 1, 0, "C"
                    )
                else:
...
def verify_huddle_test(
    ...
    if (
        False
    ):  ## set default input values in development; set to True if running the code line-by-line
    ...
    gps_ax = gps_fig.subplots(len(SN_list), squeeze=False)[
        :, 0
    ]  # necessary to make subplots work for single-logger test
    ...
    for SN_index, SN in enumerate(SN_list):
        ...
        interval_dict[SN] = np.nanmean(
            np.diff(metadata.t)
        )  # calculate interval between metadata sampling. use nanmean in case of possible missing times.
        ...
        if pstats_df.loc[SN, "Battery min"] < batt_min:
            ...
            _metadata_status(
                "error", err_message, errors, SN, dataframe=errors_df, col_name="Battery min"
            )
            ...
            _metadata_status(
                "warning", warn_message, warnings, SN, dataframe=errors_df, col_name="Battery min"
            )
        else:
        ...
        if pstats_df.loc[SN, "Battery max"] > batt_max:
            ...
            _metadata_status(
                "error", err_message, errors, SN, dataframe=errors_df, col_name="Battery max"
            )
            ...
            _metadata_status(
                "warning", warn_message, warnings, SN, dataframe=errors_df, col_name="Battery max"
            )
        else:
        ...
        if pstats_df.loc[SN, "Temperature min"] < -20:  # degrees Celsius
            ...
            _metadata_status(
                "error", err_message, errors, SN, dataframe=errors_df, col_name="Temperature min"
            )
        elif (
            pstats_df.loc[SN, "Temperature min"] < -15
        ):  # modify as needed, just a backbone structure for now.
        ...
        if pstats_df.loc[SN, "Temperature max"] > 60:  # degrees Celsius
            ...
            _metadata_status(
                "error", err_message, errors, SN, dataframe=errors_df, col_name="Temperature max"
            )
        elif (
            pstats_df.loc[SN, "Temperature max"] > 50
        ):  # modify as needed, just a backbone structure for now.
        ...
        if pstats_df.loc[SN, "A2 flat"] > 0.99:
            ...
            _metadata_status(
                "error", err_message, errors, SN, dataframe=errors_df, col_name="A2 flat"
            )
            ...
            _metadata_status(
                "warning", warn_message, warnings, SN, dataframe=errors_df, col_name="A2 flat"
            )
        else:
        ...
        if not within_A2_range:
            ...
            _metadata_status(
                "error", err_message, errors, SN, dataframe=errors_df, col_name="A2 range"
            )
        else:
        ...
        if pstats_df.loc[SN, "A3 flat"] > 0.99:
            ...
            _metadata_status(
                "error", err_message, errors, SN, dataframe=errors_df, col_name="A3 flat"
            )
            ...
            _metadata_status(
                "warning", warn_message, warnings, SN, dataframe=errors_df, col_name="A3 flat"
            )
        else:
        ...
        if not within_A3_range:
            ...
            _metadata_status(
                "error", err_message, errors, SN, dataframe=errors_df, col_name="A3 range"
            )
        else:
        ...
        fifo_exceeds_soft_limit = np.sum(metadata.maxFifoUsed > fifo_soft_limit) / len(
            metadata.maxFifoUsed
        )
        ...
        if fifo_exceeds_soft_limit > 0.01:
            ...
            _metadata_status(
                "note", notes_message, notes, SN, dataframe=errors_df, col_name="Unusual FIFO use"
            )
        else:
        ...
        if any(metadata.maxFifoUsed > fifo_hard_limit):
            ...
            _metadata_status(
                "note", notes_message, notes, SN, dataframe=errors_df, col_name="Excessive FIFO use"
            )
        else:
        ...
        if any(metadata.maxOverruns) != 0:
            ...
            _metadata_status(
                "warning", warn_message, warnings, SN, dataframe=errors_df, col_name="Max overruns"
            )
        else:
        ...
        if any(metadata.unusedStack1 <= 30) or any(metadata.unusedStackIdle <= 30):
            ...
            _metadata_status(
                "warning", warn_message, warnings, SN, dataframe=errors_df, col_name="Unused stack"
            )
        else:
        ...
        gps_interval_lengths = np.diff(metadata.t[metadata.gpsOnFlag == 0])[
            10:
        ]  # skip the first ten seconds and first GPS cycle, which is very long by design
        gps_interval_lengths = gps_interval_lengths[
            gps_interval_lengths > 11
        ]  ## old firmwares sampled metadata every 10 sec, which can result in 10-second apparent GPS periods
        ...
        if any(gps_interval_lengths > 180):
            ...
            _metadata_status(
                "warning", warn_message, warnings, SN, dataframe=errors_df, col_name="GPS run time"
            )
        else:
        ...
        if SN == SN_list[-1]:
            ...
            gps_ax[SN_index].set_xticks(
                np.arange(0, 10) * 20, [0, 20, 40, 60, 80, 100, 120, 140, 160, ">180"]
            )  ## 2022-06-14 JFA; seems to work
            gps_ax[SN_index].annotate(
                "GPS on-time ratio",
                (gps_proportion, 10),
                xytext=(gps_proportion + 10, 15),
                color="r",
                arrowprops=dict(
                    arrowstyle="->",
        ...
        lon_deg_to_meters = (
            40e6 / 360 * np.cos(np.median(gps.lat) * np.pi / 180)
        ...
        t = np.arange(
            0, max_seconds, d
        )  # create evenly spaced time values from 0 until the maximum to match to data
        if SN_index == 0:
        ...
        trace.normalize()
        tr_offset = (
            trace.data / 2 + SN_index
        )  # normalized scales to [-1, 1]; divide by 2 to prevent overlap
    ...
    temp_end = (
        gps_stop_max - mod - check_interval * 10
    )  # stop time on an even minute, ten minutes before end

    # Create dataframe to house temperatures to check
    column_index = np.arange(
        0, int((temp_end - temp_start) / df_width), 1
    )  # theoretically, the number of values between start and end

    group_temp_df = pd.DataFrame(
        index=SN_list, columns=column_index
    )  # create dataframe to house temperatures at each minute for each SN
    ...
    for SN_index, SN in enumerate(SN_list):
        ...
        for time in times:  # for each time value in time metadata
            argstart_list.append(
                np.abs(temp_start - time)
            )  # create a list of time values minus start time (find closest to start)
            argend_list.append(
                np.abs(temp_end - time)
            )  # create list of time values minus end time (find closest index to end)
        ...
        times_to_check_index = np.arange(
            start_index, stop_index, temp_interval
        )  # create evenly spaced array of even minutes
        times_checked = np.arange(
            temp_start, temp_end, temp_interval
        ...
        for df_index, index in enumerate(times_to_check_index):
            ...
            group_temp_df.iloc[SN_index, df_index] = metadata.temp[
                index
            ]  # save minute temperature reading into dataframe
    error = False
    for (
        column
    ) in column_index:  # column represents temperature data for each time that will be checked
        ...
        outliers = np.where(
            any(group_temp_df[column]) > temp_median + 1.5
            or any(group_temp_df[column] < temp_median - 1.5)
        )[0]
    ...
    info.append(
        f"""BATTERY TEST INFO: This test ensures the voltage of each Gem is within {batt_min} to {batt_max} Volts at each recorded instance. Gems within this specified range, but within a range 0.5 Volts from the threshold will result in a warning message. See battery voltage figure for details."""
    )
    trouble.append(
