...
import obspy, glob, gemlog, pathlib
...
def check_discontinuity(files):
    ...
    if len(st) > 1:
        for tr in st:
            print(
                tr.id + " " + tr.stats.starttime.isoformat() + "--" + tr.stats.endtime.isoformat()
            )
...
def check_lags(DB, winlength=1000, fl=0.5, fh=20, maxshift=10, verbose=False):
    ...
    while t1 < max(DB.t2):
        ...
        try:
            ...
            for i in range(nsta):
                xc_output = xcorr_max(
                    correlate(st_test.traces[i], st_test.traces[(i + 1) % nsta], maxshift)
                )
...
def make_db(path, pattern="*", savefile=None, verbose=False):
    ...
    for file in files:
        ...
        row = pd.DataFrame(
            ...
            columns=["filename", "station", "location", "amp_HP", "max", "min", "t1", "t2"],
        )
        DB.append(row)
...
def calc_channel_stats(DB, t1, t2):
    ...
    DB.t2 = DB.t2.apply(T)
    DB.goodData = (
        (DB.amp_HP > 0.5) & (DB.amp_HP < 2e4) & ((DB.t2 - DB.t1) > 3598) & ((DB.t2 - DB.t1) < 3602)
    )
            ...
            out.append(
                pd.DataFrame(
                    [
                        [
                            sta,
                            np.sum(np.array(DB.goodData)[w]) / numHour,
                            np.sum(np.array(DB.anyData)[w]) / numHour,
                            q1,
                            q3,
                        ]
                    ],
                    columns=["station", "goodData", "anyData", "q1", "q3"],
                )
            )
    out = pd.concat(out)
    return out
...
_noise_path = pathlib.Path(__file__).parent / "data" / "noise"
...
def gem_noise(freq=None, spectype="power", version="1.0", freq_min=None, freq_max=50):
    ...
    if float(version) >= 0.97:
        noise_spec = pd.read_csv(_noise_path / "Gem_v0.98_Noise_spec.txt")
    else:
...
def ims_noise(model="low", freq=None, spectype="power", freq_min=None, freq_max=50):
    ...
    # 2: wants output frequencies and spectrum with no frequency input
    # in order to permit 2, we need to return frequencies and spectrum
    noise_spec = pd.read_csv(
        _noise_path / "IMSNOISE_MIN_MED_MAX.txt", names=["f", "min", "med", "max"], sep=r"\s+"
    )
    freq_in = noise_spec["f"]
...
def _noise_spectrum_helper(freq_in, power_in, freq_out, spectype, freq_min, freq_max):
    ...
    if (freq_min is not None) and (freq_max is not None):
        noise = scipy.integrate.quad(spec_function, freq_min, freq_max)[
            0
        ]  # 'quad' outputs are the estimated integral and error bar
    else:
        noise = np.NaN
...
def _interpolate_stream(st, gap_limit_sec=0.1):
    ...
    for i in range(len(st) - 1):
        ...
        if (t2 - t1) < gap_limit_sec:
            ...
            interp_function = scipy.interpolate.CubicSpline(
                t[data_good], tr_new.data[data_good], extrapolate=False
            )
    ...
    st = (
        st.split()
    )  ## if there are still data gaps, break them up (ensuring that the result is not a masked array)
