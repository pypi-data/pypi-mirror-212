...
def get_gem_response(gain="high", sensor_file="", logger_file=""):
    ...
    response.instrument_sensitivity.value = (
        response.response_stages[0].stage_gain
        * response.response_stages[2].stage_gain
        * filter_gain_0_05
    )
    return response
...
def _get_station_info(station_info):
    ...
    if type(station_info) == str:
        ...
        if all(
            [i.lower() in (required_keys + ["elevation"]) for i in header_list]
        ):  # file has header line
            print("File has valid header, using that for column names")
            station_info = pd.read_csv(
                station_info, dtype={key: "str" for key in header_list}, keep_default_na=False
            )
        else:
            if len(header_list) == 4:
                print(
                    "File does not have a valid header, using default columns [SN, network, station, location]"
                )
                station_info = pd.read_csv(
                    station_info,
                    names=required_keys,
                    dtype={key: "str" for key in header_list},
                    keep_default_na=False,
                    index_col=False,
                )
            elif len(header_list) == 5:
                print(
                    "File does not have a valid header, using default columns [SN, network, station, location, elevation]"
                )
                station_info = pd.read_csv(
                    station_info,
                    names=required_keys + ["elevation"],
                    dtype={key: "str" for key in header_list},
                    keep_default_na=False,
                    index_col=False,
                )
                ...
                raise Exception(
                    "invalid station_info file; must have 4 or 5 columns or valid header"
                )
        ## if any keys are capitalized or abbreviated wrong, correct them
        station_info = _fix_station_info_keys(station_info)
    elif (type(station_info) is not pd.DataFrame) or any(
        [key not in station_info.keys() for key in required_keys]
    ):
...
def make_gem_inventory(station_info, coords, response="default"):
    ...
    if type(coords) is not pd.DataFrame:
        ...
        [key not in coords.keys() for key in ["network", "station", "location"]]
    ):
        raise Exception(
    ...
    for network_name in all_networks:
        ...
        for station_name in all_stations:
            station = obspy.core.inventory.Station(
                station_name, latitude=0, longitude=0, elevation=0
            )
            ...
            for location_name in all_locations:
                ...
                SN = station_info[
                    (station_info["network"] == network_name)
                    & (station_info["station"] == station_name)
                    & (station_info["location"] == location_name)
                ]["SN"].iloc[0]
                ## find the coords line corresponding to this location
                if ("SN" in coords.keys()) and any(
                    key not in coords.keys() for key in ["network", "station", "location"]
                ):
                    line = coords[
                        coords["SN"].astype(int) == int(SN)
                    ]  # int just in case there's some type discrepancy
                else:
                ...
                if line.shape[0] == 0:
                    ...
                    print(
                        f"No coords found for {network_name}.{station_name}.{location_name}, skipping"
                    )
                    continue
                ...
                equipment = obspy.core.inventory.util.Equipment(
                    serial_number=SN,
                    model="Gem Infrasound Logger v1.0",
                    description="Gem 1.0 (Infrasound), 0.039-27.1 Hz, 0.0035012 Pa/count",
                )
...
def merge_files_day(infile_path, infile_pattern="*", outfile_dir="merge_file_output"):
    ...
    cuts = [
        infile.split("/")[-1].split(".")[0].split("T")[0]
        + ","
        + ".".join(infile.split("/")[-1].split(".")[1:])
...
def _remove_outliers(x, N=5):
    w = np.where(
        (np.abs(x.lat - np.median(x.lat)) < (N * (1e-5 + np.std(x.lat))))
        & (np.abs(x.lon - np.median(x.lon)) < (N * (1e-5 + np.std(x.lon))))
    )[0]
    if len(w) < x.shape[0]:
...
def read_gps(gps_dir_pattern, SN):
    ...
    for gps_dir in gps_dir_list:
        ...
        if len(fn_list) > 0:  # if any gps files matching SN are found, read and append the last
            gps_table = pd.concat([gps_table, pd.read_csv(fn_list[-1])], ignore_index=True)
                ...
                [
                    ...
                    pd.DataFrame(
                        [
                            [
                                SN,
                                avg_fun(gps_table.lat),
                                avg_fun(gps_table.lon),
                                se_fun(gps_table.lat),
                                se_fun(gps_table.lon),
                                gps_table.t.min(),
                                gps_table.t.max(),
                                gps_table.shape[0],
                            ]
