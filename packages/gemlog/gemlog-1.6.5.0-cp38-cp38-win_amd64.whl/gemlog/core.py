...
def convert(
    ...
    if len(SN) != 3:
        raise Exception(
            f"Invalid serial number; SN type is length-{str(len(SN))} {str(type(SN))}, not length-3 str"
        )
    ...
    if (type(SN) is not str) or (
        len(SN) != 3
    ):  # check that SN is appropriately formatted...this should always happen
        fn = glob.glob(rawpath + "/FILE" + "[0-9]" * 4 + ".???")
    else:
        fn = glob.glob(rawpath + "/FILE" + "[0-9]" * 4 + "." + SN) + glob.glob(
            rawpath + "/FILE" + "[0-9]" * 4 + ".TXT"
        )
    ...
    if len(nums) == 0:
        if corrupt_files_flag:
            raise CorruptRawFile(
                f'No non-corrupt data files for SN "{SN}" found in raw directory {rawpath}'
            )
        else:
    ...
    while (L["data"].count() == 0) & (
        n1 <= max(nums)
    ):  ## read sets of files until we get one that isn't empty
        nums_block = nums[
            (nums >= n1) & (nums < (n1 + (12 * blockdays)))
        ]  # files are 2 hours, so 12 files is 24 hours
    ...
    if np.isinf(float(t2)):
        t2 = obspy.core.UTCDateTime.strptime(
            "9999-12-31 23:59:59", "%Y-%m-%d %H:%M:%S"
        )  # timekeeping apocalypse

    wsn = 0
    while (
        len(SN) < 3
    ):  # take the first non-NA SN. This is important because there can be blank files in there.
        wsn = wsn + 1
        SN = L["header"]["SN"][wsn]
    ...
    gps = L["gps"]
    metadata.to_csv(
        metadatafile, index=False
    )  ## change to metadata format. need to make ScanMnetadata compatible with both
    ...
    while True:
        ...
        while (p[-1].stats.endtime < tt2) & (n1 <= max(nums)):
            try:
                L = read_gem(
                    path=rawpath, nums=nums[(nums >= n1) & (nums < (n1 + (12 * blockdays)))], SN=SN
                )
            ...
            if len(L["data"]) > 0:
                if (
                    L["data"][0].stats.starttime - p[-1].stats.endtime
                ) <= 0.031:  # interpolate a gap up to 3 samples
                    L["data"] += p[-1]
        ...
        while (hour_to_write + file_length_sec) <= p[-1].stats.endtime:
            hour_to_write = _write_hourlong_mseed(
                p,
                hour_to_write,
                file_length_sec,
                bitweight,
    ...
    while (hour_to_write <= p[-1].stats.endtime) & (len(p) > 0):
        hour_to_write = _write_hourlong_mseed(
            p, hour_to_write, file_length_sec, bitweight, convertedpath, output_format=output_format
        )
        p.trim(hour_to_write, t2)
        p = p.split()
        if len(p) > 0:
...
def _write_hourlong_mseed(
    ...
    pp = p.slice(
        hour_to_write, hour_end - eps, nearest_sample=False
    )  # nearest sample and eps avoid overlapping samples between files
    ...
    for tr in pp:
        ...
        if len(tr) > 0:
            ...
            if output_format.lower() == "wav":
                ...
                tr.write(
                    convertedpath + "/" + fn, format=output_format, encoding=10
                )  # encoding 10 is Steim 1
...
def read_gem(
    ...
    if version in ["0.85C", "0.9", "0.91", "1.10"]:
        ...
        L = _read_several(
            fn_list, version=version, require_gps=require_gps
    ...
    if len(L["data"]) == 0:
        raise CorruptRawFileInadequateGPS(
            f'Inadequate GPS information in data files {str(nums)} for SN "{SN}" in raw directory {str(path)}'
        )
...
def _new_gem_var():
    ...
    output = {"data": tr, "metadata": metadata, "gps": gps}
    return output


def _read_SN(fn):
    try:
        SN_lines = pd.read_csv(
            fn,
            delimiter=",",
            skiprows=3,
            nrows=4,
...
def _read_config(fn):
    ...
    )  ## default config: it's fairly safe to use this as the default because any other configuration would require ...
...
def _find_nonmissing_files(path, SN, nums):
    ...
    for fn in good_fn_list:
        if (
            pathlib.Path(fn).stat().st_size > 10
        ):  # to be safe, anything under 10 bytes is treated as empty
...
def _check_gps(line):  # return True if GPS line is good
    ...
    return not (
        ...
        or (line[7] > 60)  # minute
        or (line[7] < 0)
        or (line[7] != np.round(line[7]))
    )  # second


def _make_gps_time(line):
    try:
        return obspy.UTCDateTime(
            int(line[2]), int(line[3]), int(line[4]), int(line[5]), int(line[6]), int(line[7])
        )
    except:
        return np.NaN
...
def _read_0_8_with_pandas(filename, require_gps=True):
    ...
    try:
        df = pd.read_csv(
            filename, names=range(13), low_memory=False, skiprows=6, encoding_errors="ignore"
        )
    except Exception:
        try:
            df = pd.read_csv(
...
def _valid_gps(G):
    ...
    # custom limits:
    bad_gps |= (G["lat"] == 0) | (G["lon"] == 0) | (G["second"] != np.round(G["second"]))
    return ~bad_gps
    ...
    with open(filename, "r", newline="", encoding="ascii", errors="ignore") as csvfile:
        ...
        for line in lines:
            ...
            try:
                line_type = line[0][
                    0
                ]  # if this fails, it means the line is empty or at least invalid
            except:
                continue
...
def _calculate_drift(L, fn, require_gps):
    ...
    if ("t" not in L["gps"].keys()) or (len(L["gps"].t) == 0):
        ...
        sufficient_gps = (
            0.001024
            * (L["data"][-1, 0] - L["data"][0, 0])
            / (L["gps"].t.iloc[-1] - L["gps"].t.iloc[0])
        ) < 2
    ...
    if sufficient_gps:
        try:
            ...
            reg, num_gps_nonoutliers, MAD_nonoutliers, resid, xx, yy = _robust_regress(
                L["gps"].msPPS, L["gps"].t
            )
            ## ensure that the regression was successful
            if (
                ((0.001024 * (L["data"][-1, 0] - L["data"][0, 0]) / (xx.iloc[-1] - xx.iloc[0])) > 2)
...
def _robust_regress(x, y, z=4, recursive_depth=np.inf, verbose=False):
    ...
    if any(outliers) and (recursive_depth > 0):
        ...
        return _robust_regress(
            x[~outliers], y[~outliers], z, recursive_depth=recursive_depth - 1, verbose=verbose
        )
    else:
...
def _plot_drift(file_list, z=4, recursive_depth=np.inf):
    ...
    plt.plot(
        xg, _apply_segments(xg, output["header"]) - _no_drift(xg) - yg[0] - xg * drift_slope, "k."
    )
...
def _apply_fit(x, model):
    return (
        model["drift_deg0"]
        + model["drift_deg1"] * x
        + model["drift_deg2"] * x**2
        + model["drift_deg3"] * x**3
    )
...
def _assign_times(L):
    ...
    try:
        ...
        raise CorruptRawFile(
            f"Problem between {fn_list[0]}-{fn_list[-1]}; stopping before this interval. Break between recording periods? Corrupt files?"
        )
    ...
    L["data"] = _interp_time(
        D
    )  # returns stream, populates known fields: channel, delta, and starttime
    L["gps"] = G
...
def _interp_time(data, t1=-np.Inf, t2=np.Inf, min_step=0, max_step=0.025):
    ...
    t2 = t_in[t_in <= (t2 + 0.01 + eps)][-1]  # add a sample because t2 is 1 sample before the hour
    breaks_raw = np.where((np.diff(t_in) > max_step) | (np.diff(t_in) < min_step))[
        0
    ]  # 2020-11-04: check for backwards steps too
...
def get_gem_specs(SN):
    ...
    version_table = {
        "version": np.array(
            [0.5, 0.7, 0.8, 0.82, 0.9, 0.91, 0.92, 0.98, 0.99, 0.991, 0.992, 1, 1.01]
        ),
    ...
    version = version_table["version"][
        (int(SN) >= version_table["min_SN"]) & (int(SN) <= version_table["max_SN"])
    ][0]
    bitweight_V = 0.256 / 2**15 / __gain__(version)
    sensitivity = (
        __AVCC__(version) / 7.0 * 45.13e-6
    )  # 45.13 uV/Pa is with 7V reference from Marcillo et al., 2012
    return {
        "version": version,
...
def _reformat_GPS(G_in):
    ...
    G_dict = {
        "year": np.array([int(year) for year in G_in.year]),
        "date": np.array(date),
...
def _find_breaks(L):
    ...
    try:
        badTags = (
            (t > obspy.UTCDateTime.now())
            | (L["gps"].year < 2015)  # no future dates
            | (  # no years before the Gem existed
                (np.abs(t - tPad[:-2]) > 86400) & (np.abs(t - tPad[2:]) > 86400)
            )
            | (L["gps"].lat == 0)  # exclude outliers
            | (L["gps"].lon == 0)  # exclude points within ~1m of the equator
        )  # exclude points within ~1m of the prime meridian
    ...
    dmG_dtG = (
        np.diff(mG) / np.diff(tG) * 1.024
    )  # correction for custom millis in gem firmware (1024 us/ms)
    gps_breaks = np.argwhere(
        np.isnan(dmG_dtG)
        | ((np.diff(tG) > 50) & ((dmG_dtG > 1000.1) | (dmG_dtG < 999.9)))  # missing data...unlikely
        | (  # 100 ppm drift between cycles
            (np.diff(tG) <= 50) & ((dmG_dtG > 1002) | (dmG_dtG < 998))
        )  # most likely: jumps within a cycle (possibly due to leap second)
    )
    min_possible_start = (
        mD.min()
    )  # this only changes if a GPS break around the first GPS sample invalidates preceding D samples
    for i in gps_breaks:
        i = int(i)
        ...
        if np.any(overlaps):
            ...
            try:
                starts = np.append(
                    starts, mG[wmin][tG[wmin] == tG[wmin].min()][-1]
                )  # [-1] just in case tG values are repeated
            except:
                _breakpoint()
            ...
            if len(wmax) == 0:  # empty wmax means no fixes before break
                ...
                ends = np.append(
                    ends, mG[wmax][tG[wmax] == tG[wmax].max()][-1]
                )  # [-1] in case tG values are repeated
...
def _convert_one_file(input_filename, output_filename=None, require_gps=True):
    try:
        ...
        if D.shape[0] == 0:
            raise CorruptRawFileNoGPS(
                f'Failed to convert file "{input_filename}", due to no GPS data'
            )
        ...
        if require_gps:
            ...
            raise CorruptRawFileNoGPS(
                f'Failed to convert file "{input_filename}" due to no GPS data, even though GPS data is not needed. This should not happen and indicates a bug.'
            )

    except CorruptRawFile:
        if require_gps:
            raise CorruptRawFile(
                f'Failed to convert file "{input_filename}", possibly due to inadequate GPS data'
            )
        else:
