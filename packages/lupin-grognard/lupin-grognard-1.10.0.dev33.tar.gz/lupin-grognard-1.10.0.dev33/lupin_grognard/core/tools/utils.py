...
def generate_commits_range_to_check(
    ...
    """
    merge_count = 0
    print(f"is ci mr target branch main: {is_ci_mr_target_branch_main}")
            ...
            for branch in branch_list:
                if f"into '{branch}'" in commit_obj.title:
                    ...
                    if is_ci_mr_target_branch_main and merge_count == 2:
                        print(f"Found merge commit: {commit_obj.title}")
                        index = commit_list.index(commit)
                        return commit_list[:(index - 1)] # -1 to remove the merge commit
