        ...
        if mxl_init and init_class_betas is None:
            ...
            for i in range(num_classes):
                ...
                mxl.fit(X_mxl,
                        ...
                        verbose=0,
                        gtol=gtol)
                init_class_betas[i] = mxl.coeff_ + dev.np.random.normal(0, 0.01, len(mxl.coeff_))

        print("finished MXL coeffs")

        self.init_class_betas = dev.np.array(init_class_betas)
        self.init_class_thetas = dev.np.array(init_class_thetas)
        print("finished MXL coeffs (2)")
