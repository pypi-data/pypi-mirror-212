...
class TestContractSearchController(BaseEMCRestCaseAdmin):
    ...
    def test_route_contract_search_to_dict_i18n_es(self):
        self.contract.contract_line_ids[0]["product_id"] = self.browse_ref(
            "somconnexio.ADSL20MB100MinFixMobile"
        )
        self.url = "{}?{}={}".format(self.url, "code", self.contract.code)
        response = self.http_get_with_lang("es")
        self.assertEquals(response.status_code, 200)
        result = json.loads(response.content.decode("utf-8"))

        self.assertEquals(result["contracts"][0]["id"], self.contract.id)
        self.assertEquals(
            result["contracts"][0]["description"], "ADSL 100 min a fijo o móvil"
        )

    def test_route_contract_search_to_dict_i18n_ca_es(self):
        self.contract.contract_line_ids[0]["product_id"] = self.browse_ref(
            "somconnexio.ADSL20MB100MinFixMobile"
        )
        self.url = "{}?{}={}".format(self.url, "code", self.contract.code)
        response = self.http_get_with_lang("ca_ES")
        self.assertEquals(response.status_code, 200)
        result = json.loads(response.content.decode("utf-8"))

        self.assertEquals(result["contracts"][0]["id"], self.contract.id)
        self.assertEquals(
            result["contracts"][0]["description"], "ADSL 100 min a fix o mòbil"
        )
