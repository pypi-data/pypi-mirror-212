...
class cryst_struct(object):
    ...
    def find_symmetry(self,symmprec=1e-4,report=False):
        if self._is_slab: print ('slab structure, be cautious when using the 3D space group for symmetry analysis!\n')
        spg = None
        spg_no = 0
        try:
    ...
    def write_wien2k_struct(self,case='case',title='TITLE',symmprec=1E-4):
        ...
        ibrav,brav,center=self._get_ibrav(symmprec=symmprec)
        spg,spg_no=self.find_symmetry(symmprec=symmprec)
