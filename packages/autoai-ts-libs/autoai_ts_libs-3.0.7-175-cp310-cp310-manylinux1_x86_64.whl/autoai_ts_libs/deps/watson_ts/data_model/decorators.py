...
def _arg_converter_decorator(
    ...
    **bound_converter_kwargs,
    ...
    def decorator(wrapped, instance, args, kwargs):
        ...
        # Local
        from autoai_ts_libs.deps.watson_ts.base_mixins import SKLearnEstimatorWrapper

        if isinstance(instance, SKLearnEstimatorWrapper) and not is_type_to_ignore:
        ...
        if arg_val is not None and not is_type_to_ignore:
            ...
            if isinstance(instance, SKLearnEstimatorWrapper):
        ...
        if is_type_to_ignore:
            ...
            if (
                isinstance(instance, SKLearnEstimatorWrapper)
                and wrapped_output is not instance
            ):
                # get only those values from the signature that are not empty (contain defaults)
                updated_kwargs = {
                    k: v.default
                    for k, v in inspect.signature(wrapped).parameters.items()
                    if v.default != inspect._empty
                }
                # overwrite these defaults if kwargs already has them
                updated_kwargs.update(kwargs)
                # convert to df with the underlying instance implementation from SKLearnEstimatorWrapper
                dm_args, df_out = instance._map_to_df(
                    updated_arg_val,
                    wrapped_output,
                ...
                # arg_val
                return inverse_conversion_func(df_out, shape_change, **dm_args)
            else:
                # non sklearn types
                return inverse_conversion_func(wrapped_output, shape_change)
        else:
