...
from dartrig.parser import dsaf_parse_nodes
...
def get_node_text_contains(nodes, search_text):
    for node in nodes:
        node_txt = remove_strs_list(node.get("text"), [" "])
...
class DartWeb:
    ...
    def get_dsaf_meta(self, company, rcp_no) -> Dict[str, any]:
        ...
        try:
            ...
            html_text = self._try_file_cache_request(f"{URLS['DART_DSAF']}?rcpNo={rcp_no}", "dsaf", rcp_no)
            nodes = dsaf_parse_nodes(html_text)

            c_type = 'gb'
            dcm_no, ele_id, offset, length = ['-1'] * 4

            if ('금융지주' in company) or ('인베스트먼트' in company) or ('보험' in company[-2:]) or ('증권' in company) or (
                    '금융투자' in company) or ('뱅크' in company[-2:]) or ('신탁' in company[-2:]) or ('생명' in company[-2:]):
                c_type = 'fin'
            ...
            meta = { "c_type": c_type, "dcm_no": dcm_no, "ele_id": ele_id, "offset": offset, "length": length }
