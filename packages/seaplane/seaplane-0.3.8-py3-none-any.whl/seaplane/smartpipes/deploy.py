...
def create_carrier_workload_file(smart_pipe_id: str, coprocessor: Coprocessor, next_coprocessors: List[str]) -> None:
    output = None

    if len(next_coprocessors) > 1:
        output = {
            "broker": {
            "outputs": map(lambda c_id: {"carrier": {"subject": f"{smart_pipe_id}.{c_id}" } },  next_coprocessors)
            }
        }
        ...
        output = {
            "label": "carrier_out",
            "carrier": {
                "subject": f"{smart_pipe_id}.{next_coprocessors[0]}"
            }
        }
    ...
    workload = {
        ...
        "processor": {
            "docker": {
            "image": f"us-central1-docker.pkg.dev/artifacts-356722/coprocessors/{coprocessor.id}:latest",
            "args": []
            }
        },
        "output": output
...
def create_stream(name: str) -> Any:
    url = f"{config.carrier_endpoint}/streams/{name}"
...
def delete_flow(name: str, workload: Dict[str, Any]) -> Any:
    url = f"{config.carrier_endpoint}/flow/{name}"
    req = provision_req(config._token_api)
...
def deploy() -> None:
    ...
    for sm in context.smart_pipes:
      ...
      for c in sm.coprocessors:
          ...
          print(f"Created {c.id} Dockerfile and workloads")
