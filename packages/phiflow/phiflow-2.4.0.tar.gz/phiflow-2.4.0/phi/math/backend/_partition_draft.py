...
def find_neighbors_semi_sparse(positions,
    ...
    if not b.is_available(positions):
        assert max_neighbors is not None, f"pair_count must be provided when tracing find_neighbors"
    positions = b.to_float(b.as_tensor(positions))
    num_part, d = b.staticshape(positions)
    cell_ids, perm, cell_size, full_pos, full_cell_ids = build_cells_semi_sparse(positions, cutoff, domain, periodic, cell_mode, max_neighbors)
    # b_ = choose_backend(cell_size)
    # linear_indices = b.range(num_part, dtype=index_dtype)
    # particle_ids = b.gather(linear_indices, perm, 0)

    # --- cell_list_candidate_fn ---
    # full_cell_ids = cell_idx in Jax-MD cell_list_candidate_fn()

    neighbor_cell_idx = ...  # ell_idx
    # for every shift (-1, -1), (...
        # shift full_cell_ids
        # add to list
    neighbor_cell_idx = b.concat(neighbor_cell_idx, -2)
    neighbor_idx = b.zeros((num_part + 1, neighbor_cells, max_neighbors))
    neighbor_idx = b.scatter(neighbor_idx, b.reshape(full_cell_ids), b.reshape(neighbor_cell_idx))

    # --- prune_neighbor_list_sparse ---
    # idx = neighbor_idx
    sender_idx = b.flatten(b.broadcast_to(b.range(num_part), neighbor_cells, max_neighbors))  # (N * 27)
    receiver_idx = b.flatten(neighbor_idx)
    dx = positions[sender_idx] - positions[receiver_idx]
    mask = (b.sum(dx ** 2) < cutoff ** 2) & (receiver_idx < num_part)
    cumsum = b.cumsum(mask)
    index = b.where(mask, cumsum - 1, len(receiver_idx) - 1)
    out_idx = num_part * b.ones(receiver_idx, index_dtype)
    receiver_idx = b.scatter(out_idx, index, receiver_idx)
    sender_idx = b.scatter(out_idx, index, sender_idx)
    max_occupancy = cumsum[-1]
    return sender_idx[:, :max_occupancy], receiver_idx[:, :max_occupancy], dx

def build_cells_semi_sparse(positions,
                min_cell_size,
                domain: Optional[Tuple[TensorType, TensorType]],
                periodic: Union[bool, Tuple[bool, ...]],
                mode: str,
                max_neighbors) -> Tuple[TensorType, TensorType, TensorType, TensorType, 'IndexingStructure']:
    ...
    if mode == 'grid':
        ...
        perm = b.argsort(cell_ids)  # sort_map in Jax-MD
        cell_indices = b.gather(cell_indices, perm, 0)
        cell_ids = b.gather(cell_ids, perm, 0)
        sorted_pos = b.gather(positions, perm, 0)
        position_in_full_array = max_neighbors * cell_ids + (b.range(n) % max_neighbors)  # sorted cell ids
        full_pos = b.zeros((1, cell_count * max_neighbors, d))  # cell_position
        full_pos = b.scatter(full_pos, position_in_full_array[None, :, None], sorted_pos[None, :, :], mode='update')[0, ...]
        full_pos = b.reshape(full_pos, (cell_count, max_neighbors, d))
        full_cell_ids = b.zeros((1, cell_count * max_neighbors, 1)) - 1
        full_cell_ids = b.scatter(full_cell_ids, position_in_full_array[None, :, None], cell_ids[None, :, None], mode='update')[0, ...]
        full_cell_ids = b.reshape(full_cell_ids, (cell_count, max_neighbors, 1))  # cell_id in Jax-MD
        occupancy = b.bincount(cell_ids, None, bins=cell_count)  # len_by_cell
        max_occupancy = b.max(occupancy)
        if max_occupancy > max_neighbors:
            raise OverflowError()  # ToDo
        return cell_ids, perm, cell_size, full_pos, full_cell_ids
