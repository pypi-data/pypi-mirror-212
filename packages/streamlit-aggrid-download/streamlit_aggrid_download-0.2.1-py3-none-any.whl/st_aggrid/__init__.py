...
if not _RELEASE:
    df_ips = pd.read_csv('/Users/CTI/00_src/99_Amit/07_streamlit/99_per_client/user/Item Master Generic Content - 110123.csv')
    gb_t4 = GridOptionsBuilder.from_dataframe(df_ips)
    gb_t4.configure_default_column(groupable=True, value=True, enableRowGroup=True, aggFunc=None)
    gb_t4.configure_selection('single', use_checkbox=True, groupSelectsChildren=False, groupSelectsFiltered=False) 
    # gb_t4.configure_pagination(enabled=True, paginationAutoPageSize=False, paginationPageSize=10)
    ...
    # }
    # """)
    # gb_t4.configure_columns(['Item Code','unit_type'], aggFunc='nunique')
    gb_t4.configure_columns(['criticality','unit_type'], aggFunc=None)
    # gb_t4.configure_columns(['soh'], valueFormatter=k_sep_formatter, aggFunc="sum")

    gb_grid_options_t4 = gb_t4.build()
    grid_response4 = AgGrid(
        df_ips,
        gridOptions=gb_grid_options_t4,
