...
class BaseRequest:
    ...
    def base(self, path, func_name, params, config: Config, **kwargs):
        ...
        # 解析模版参数
        url, headers, data, templates = self.base_init_prepare_request(config, templates, func_name)

        # 创建一个新的 Context 实例
        context = Context(headers, data, url)

        # 创建 HookedRequest 实例，所有钩子类将自动注册
        generate_parameters_hook_params = {"params": params, "context": context, "url_module": templates['url'], "func_name": func_name}
    ...
    def base_init_prepare_request(self, config, templates: dict, func_name: str):
        url = self.get_template_url_prefix(templates['url'], config.env_name) + self.get_template_path(templates['url'], func_name, config.env_name)
        headers = get_template_value(templates['headers'], func_name + '_headers')
        data = get_template_value(templates['body'], func_name + '_body')
        return url, headers, data, templates
    ...
    @staticmethod
    def get_template_url_prefix(template, env_name: str):
        attribute_name = 'url_prefix' + f'_{env_name}' if hasattr(template, 'url_prefix' + f'_{env_name}') else 'url_prefix'
        return template.__getattribute__(attribute_name) if hasattr(template, attribute_name) else ''

    @staticmethod
    def get_template_path(template, func_name: str, env_name: str):
        if func_name.startswith(FUNC_NAME_PREFIX):
            func_name = func_name.replace(FUNC_NAME_PREFIX, '')
        attribute_name = func_name + '_path' + f'_{env_name}' if hasattr(template, func_name + '_path' + f'_{env_name}') else func_name + '_path'
        return template.__getattribute__(attribute_name) if hasattr(template, attribute_name) else ''
...
class GenerateParametersHook(Hook):
    def __init__(self, config, params, context, url_module, func_name, priority=0):
    ...
    def execute(self):
        ...
        if '_url' in self.params:
            if hasattr(self.url_module, self.func_name + '_path_params'):
                path_params = getattr(self.url_module, self.func_name + '_path_params')
...
class SendRequestHook(Hook):
    ...
    def make_request(self, request_method):
        ...
        return self.session.request(request_method, self.context.url, headers=self.context.headers, data=self.context.data)
