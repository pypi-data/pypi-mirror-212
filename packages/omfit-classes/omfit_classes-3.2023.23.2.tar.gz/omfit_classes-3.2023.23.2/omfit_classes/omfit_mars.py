...
class OMFITmars(SortedDict, OMFITobject):
    ...
    def get_Xcyl(self):
        ...
        R0 = self[self.sim]['R'].isel(s=range(II))
        # Calculate Xr,z,phi
        Xr = (self['XPLASMA']['X1'].values * dRds.values + self['XPLASMA']['X2'].values * dRdchi.values) / J.values
        Xr[0:2, :] = Xr[2, :]
        Xz = (self['XPLASMA']['X1'].values * dZds.values + self['XPLASMA']['X2'].values * dZdchi.values) / J.values
        Xz[0:2, :] = Xz[2, :]
        Xphi = (self['XPLASMA']['X3'].values * R0.values) / J.values
        Xphi[0:2, :] = Xphi[2, :]
