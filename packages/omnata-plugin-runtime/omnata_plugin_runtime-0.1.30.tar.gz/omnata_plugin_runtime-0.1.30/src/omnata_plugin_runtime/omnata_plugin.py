...
from typing import Annotated, Any, Callable, Dict, Iterable, List, Literal, Optional, Type, Union
...
from .api import PluginMessage, PluginMessageAbandonedStreams, PluginMessageCancelledStreams, PluginMessageCurrentActivity, PluginMessageStreamProgressUpdate, PluginMessageStreamState
...
class SyncRequest(ABC):
    ...
    def _plugin_message(self,message:PluginMessage):
        ...
        with self._snowflake_query_lock:
            try:
                # this is not ideal, but "Bind variable in stored procedure is not supported yet"
                self._session.sql(f"""call {self._source_app_name}.API.PLUGIN_MESSAGE(
                                  {self._run_id},
