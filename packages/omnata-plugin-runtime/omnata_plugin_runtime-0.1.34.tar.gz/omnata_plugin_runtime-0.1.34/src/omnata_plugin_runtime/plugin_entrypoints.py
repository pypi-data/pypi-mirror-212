    ...
    def create_billing_events(self,session, event_request:Dict):
        ...
        for billing_event in events:
            ...
            handle_proc_result(session.sql(f"""call SYSTEM$CREATE_BILLING_EVENT(
                    ...
                    '{str(billing_event.start_timestamp)}',
                    '{str(datetime.datetime.now())}',
                    {billing_event.base_charge},
                    PARSE_JSON($${json.dumps(billing_event.objects)}$$)
                    PARSE_JSON($${json.dumps(billing_event.additional_info)}$$))
                    """).collect())
