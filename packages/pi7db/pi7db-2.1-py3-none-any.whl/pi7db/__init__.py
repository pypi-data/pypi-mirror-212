...
class server():
  ...
  def start_routes(self):
    ...
    def last_hash():
      data = pickle.loads(request.get_data())
      db = pi7db(data['dbname'],data['dbpath'])
      try:return {'hash':db.read('db_Stream_Config','last_change','hash')['data']}
      except:return {'hash':'CMP_UP'}
...
class pi7db:
  ...
  def write_log(self,coll_name,file_name,xdata,action):
    ...
    self.write('db_Stream_Config','status',{'pending':True,'hash':l_hash},False)
  ...
  def get_last_hash_server(self):
    try:
      ...
      if res.json()['hash'] == 'CMP_UP':
        self.complete_upload()
        return self.lhash
      else:return res.json()['hash']
    except:
