...
from kivy.graphics import (
    BindTexture, ClearBuffers,
    ClearColor, Color, Fbo,
...
class FrostedGlass(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self._refresh_effect_ev = Clock.create_trigger(self.refresh_effect, 0.033333, True)
    ...
    def _update_glsl(self, *args):
        ...
        self.h_blur_translate.y = self.v_blur_translate.y = (-self._pos[1] - self.size[1])
    ...
    def on_parent(self, *args):
        ...
        for p in self.parents_list:
            if isinstance(p, ModalView):
                self.popup_parent = p
            elif isinstance(p, Screen):
                self.parent_screen = p
            elif isinstance(p, ScrollView):
