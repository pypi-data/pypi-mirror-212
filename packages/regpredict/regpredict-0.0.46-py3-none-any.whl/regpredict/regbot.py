...
class Regbot:
  ...
  def prepareInput(self):
    try:
      test_data = np.array([[self.asks,self.bids,self.macd,self.rsi_05,self.rsi_15,self.close_grad,self.close_grad_neg]])
...
def signal(asks,bids,macd,rsi_05,rsi_15,close_grad,close_grad_neg):
  try:
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore")
        return Regbot(asks,bids,macd,rsi_05,rsi_15,close_grad,close_grad_neg).buySignalGenerator()
  except Exception as e:
    return {
      'Error': e
    }
