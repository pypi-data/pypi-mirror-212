...
class h5Dataset(data.Dataset):
    def __init__(self, opt):
        ...
        self.opt = opt
        self.class_label = {'d10':0, 'd25':1, 'd100': 2, 'k1':0, 'k2':1, 'k3':2, 'st1':0, 'st0.6':1, 'st2':2}
        # generate label if not merged
        if not self.opt['data_merged']:
            kernel_lbl, dose_lbl, st_lbl = self.opt['dataroot_LR'].split('/')[-1].split('_')
            self.kernel_lbl_to_assign, self.dose_lbl_to_assign, self.st_lbl_to_assign = self._cvt_int(kernel_lbl),\
    ...
    def __getitem__(self, index):
        ...
        # print('uids to open for mask and target:', uid_to_open_for_mask_target)
        ...
        if not self.opt['data_merged']:
            kernel_lbl = np.expand_dims(self.class_label[self.kernel_lbl_to_assign], axis=0)
            dose_lbl = np.expand_dims(self.class_label[self.dose_lbl_to_assign], axis=0)
        else:
            kernel_lbl = np.expand_dims(self.class_label[uid.split('_')[1:][0]], axis=0)
            dose_lbl = np.expand_dims(self.class_label[uid.split('_')[1:][1]], axis=0)
            
        # store information in dictionary 
        out_dict = {'LR': vol_in, 'HR': vol_tar, 'mask': vol_mask, 
                    'spacings': spacings, 'uid': uid, 'kernel': kernel_lbl, 'dose': dose_lbl
                    }
        return out_dict
