...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    for seriesuid in seriesUIDs:
        ...
        # print('second candidate keys:', candidates2.keys())
        ...
        # print('GT nodule exists:', len(noduleAnnots))

        # - loop over the nodule annotations
        # print('Looping over GT nodule')
        for nod_idx, noduleAnnot in enumerate(noduleAnnots):
            # increment the number of nodules
            if noduleAnnot.state == "Included":
                totalNumberOfNodules += 1

            # print('nodule :', nod_idx+1)
            x = float(noduleAnnot.coordX)
            y = float(noduleAnnot.coordY)
            z = float(noduleAnnot.coordZ)
            diameter_mm = float(noduleAnnot.diameter_mm)
            
            nod_xmin = float(noduleAnnot.xmin)
            nod_xmax = float(noduleAnnot.xmax)
            nod_ymin = float(noduleAnnot.ymin)
            nod_ymax = float(noduleAnnot.ymax)
            nod_zmin = float(noduleAnnot.zmin)
            nod_zmax = float(noduleAnnot.zmax)
            ...
            found = False
            noduleMatches = []
            # print('for nod {}, looping over candidates:'.format(nod_idx+1))
            for key, candidate in candidates.items():
                ...
                """
                cand_x = float(candidate.coordX)
                cand_y = float(candidate.coordY)
                cand_z = float(candidate.coordZ)
                
                # create a box equal to size of diam in x,y,z and check if predicted centroid lies within that box
                if (x+diameter_mm >= cand_x >= x-diameter_mm) and (y+diameter_mm >= cand_y >= y-diameter_mm) and (z+diameter_mm >= cand_z >= z-diameter_mm):
                # if (nod_xmax >= cand_x >= nod_xmin) and (nod_ymax >= cand_y >= nod_ymin) and (nod_zmax >= cand_z >= nod_zmin):
                    # print('candidate match GT for candidate key: {} | XYZ:[{},{},{}]'.format(key, candidate.coordX, candidate.coordY, candidate.coordZ))
                    if (noduleAnnot.state == "Included"):
                        found = True
                        noduleMatches.append(candidate)
                        if key not in candidates2.keys():
                            # note: if we go by detecting whether cand (x,y,z) lies within GT bbox, if two GT nodules are close to each other,
                            # same candidate can cover both GT nodule (meaning same x,y,z can be within range of two GT bbox) and in that case,
                            ...
                            # print('deleting key {} for candidates2'.format(key))
                            del candidates2[key]
    ...
    out_dict = {'CM':{'TP':candTPs, 'FP':candFPs, 'TN':candTNs, 'FN':candFNs}, 'FROC_FPs':thres, 'FROC_sens':sen,
                    'FROC_gph':fps, 'Sens_gph': sens, 'FROC_gph_itp':fps_itp, 'Sens_gph_itp':sens_itp,
                    'overall_sens': (float(candTPs) / float(totalNumberOfNodules))
               }
...
def collect(annotations_filename, valpath):
    ...
    print('Num of seriesUIDs collected:', len(seriesUIDs))
    print(seriesUIDs)
    print('-'*40)
...
def noduleCADEvaluation(annotations_filename,results_filename,valpath,outputDir, return_dict=False):
    '''
    ...
    @param outputDir: output directory
    '''
    print('GT annot:', annotations_filename)
    print('Prediction annot:', results_filename)
    print('UIDS:', valpath)
    print('-'*40)
    print('collecting nodules....')
    (allNodules, seriesUIDs) = collect(annotations_filename, valpath)
    output_dict = evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules,
