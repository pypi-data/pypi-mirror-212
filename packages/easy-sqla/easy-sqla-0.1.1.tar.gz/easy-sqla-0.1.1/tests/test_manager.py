...
class TestManager(TestCase):
    ...
    def test_operator_name_dont_collide_with_dunder_method(self):
        assert House.filter(name="Hello") == []  # because __name__  exists in every object

    def tests_get_multiple_pk(self):
        Email.create(address="anothertester123@exemple.fr", just_field="another_text", user_id=1)
        assert len(Email.get_by_pks(address="anothertester123@exemple.fr", just_field="another_text").pks) > 1

    def test_query_with_self_join(self):

        User.create(first_name="silver", last_name="rayley", parent_id={
            "id": 2000, "first_name": "roger", "last_name": "Gold D."
        })
        ...
        self.assertEqual(len(User._filter(child__parent__first_name="roger").all()), len(User.get(first_name="roger").child))
