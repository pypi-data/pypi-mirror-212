...
def get_model_attrs(model) -> OrderedDict:
    """
...
def get_primary_key(model_class: Type[DeclarativeMeta]) -> Dict:
    try:
        ...
        return {attr_name: column for attr_name, column in mapper_attrs.items() if
...
def get_operator(operator):
    ...
    """
    ALLOWED_OP = [
        'between', 'contains', 'endswith', 'ilike', 'in',
        'is', 'is_distinct_from', 'is_not', 'is_not_distinct_from', 'isnot', 'isnot_distinct_from', 'like',
        'match', 'not_ilike', 'not_in', 'not_like', 'notilike', 'notin', 'notlike', 'regexp_match', 'regexp_replace',
        'startswith', 'lt', 'le', 'eq', 'ne', 'ge', 'gt'
    ]
    if operator in ALLOWED_OP:
        ope_attr = list(
            filter(
