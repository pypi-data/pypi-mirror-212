...
class Manager:
        ...
        for field_name, field_value in data.items():
            ...
            if remote_field_model:
                ...
                if isinstance(field_value, dict):
                    ...
                    try:
                        target_col = None
                        for col in get_model_attrs(remote_field_model).get("simple_attrs"):

                            fk_target_col = list(getattr(cls, field_name).foreign_keys)[0].column
                        ...
                        if target_col is not None:
                            ...
                            try:
                                casted_value = target_col_type.process_result_value(
                                    value=field_value,
                                    dialect=cls.db_context._engine.dialect)
                        ...
                        logging.error(f"Unable to find remote keys in order to fetch {cls} instance")
                        logging.exception(exception)
                        raise exception

                if objekt:
                    if len(objekt.pks) > 1:
                        raise NotImplemented("Create object with relation to another model "
                                             "which have multiple primary key is  not supported yet.")

                    data.update({field_name: list(objekt.pks.values())[0]})

                else:
                    raise sqlalchemy.exc.NoResultFound(f"No {remote_field_model} row found with {field_value}")
    ...
    def get(cls, **conditions):
        ...
        if data:
            ...
            raise NoResultFound(f"No object {cls.__name__} found with {str(conditions)}")
