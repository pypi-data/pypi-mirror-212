...
from easy_sqla.utils import _lookup_model_manytomany_rel
from easy_sqla.utils import get_model_attrs
from easy_sqla.utils import get_model_from_rel
from easy_sqla.utils import get_operator
...
class BaseQueryBuilder:
    ...
    def _run_search(self, filters_path: Dict) -> List[Dict[str, Any]]:
        ...
        for filter_request, value in filters_path.items():
            ...
            if operator:
                operator = operator.replace("_", "")  # Bugfix
    ...
    def dive(self, model, path: List, result: Union[List, None] = None) -> Tuple:
        ...
        local_explicit_join_column = list(
            filter(
                lambda fk: getattr(fk, "name", None) or getattr(fk, "key", None) == field,
