...
from scientific_tools.graphics.function_graphs import set_label, plot_2Dfunction
import scientific_tools.physics.uncertainty as uncertainty

def plot_uncertainty_function(f, u_f, min_x, max_x, values_number,
    args_before_x=[], args_after_x=[], function_kwargs:dict={},
    title="", xlabel="", ylabel="", function_label="f(x)", uncertainty_label="f(x)±u(f(x))",
    function_plot_kwargs:dict={"color":'red', "linestyle":"-"},
    uncertainty_plot_kwargs:dict={"color":'blue', "linestyle":"-"}) :
    ...
    function_plot_kwargs is a kwargs dict to plot function
    uncertainty_plot_kwargs is a kwargs dict to plot uncertainty, ie f ± u_f (cf Matplotlib docs for futher information)
    """
    f_plus_u = lambda *args, **kwargs : f(*args, **function_kwargs, **kwargs) + u_f(*args, **function_kwargs, **kwargs)
    plot_2Dfunction(f_plus_u, min_x, max_x, values_number, args_before_x, args_after_x, function_label=uncertainty_label, **uncertainty_plot_kwargs)#draw f+u_f

    f_minus_u = lambda *args, **kwargs : f(*args, **function_kwargs, **kwargs) - u_f(*args, **function_kwargs, **kwargs)
    plot_2Dfunction(f_minus_u, min_x, max_x, values_number, args_before_x, args_after_x, **uncertainty_plot_kwargs)#draw f-u_f

    plot_2Dfunction(f, min_x, max_x, values_number, args_before_x, args_after_x, title=title, xlabel=xlabel, ylabel=ylabel, function_label=function_label, **function_plot_kwargs)#draw f (this is the last function drawing else title and axes labels haven't been displayed)
...
def plot_z_score_graph(f1, u_f1, f2, u_f2,
    min_x, max_x, values_nb, args_f1_before_x=[], args_f1_after_x=[], kwargs_f1={}, args_f2_before_x=[], args_f2_after_x=[], kwargs_f2={}, z_score_limit=2,
    title="", xlabel="", ylabel="",
    limit_label="Limits of z-score validity", z_score_label="Z-score",
    limit_plot_kwargs: dict={"color":'red', "linestyle":"-"},
    z_score_plot_kwargs:dict={"color":'blue', "z_score_linestyle":"-"}) :
    ...
    limit_plot_kwargs is a dict with kwargs to plot limit lines
    z_score_plot_kwargs is a dict with kwargs to plot z_score (cf Matplotlib docs for futher information)
    """
    ...
    #displaying
    plt.plot(x_values, z_score_values, label=z_score_label, **z_score_plot_kwargs)
    plt.plot([np.min(x_values), np.max(x_values)], [z_score_limit, z_score_limit], label=limit_label, **limit_plot_kwargs)
    plt.plot([np.min(x_values), np.max(x_values)], [-z_score_limit, -z_score_limit], **limit_plot_kwargs)
    plt.legend()
...
def plot_z_score_points_graph(x, y1, u_y1, y2, u_y2,
    z_score_limit=2,
    title="", xlabel="", ylabel="",
    limit_label="Limits of z-score validity", z_score_label="Z-score",
    limit_plot_kwargs: dict={"color":'red', "linestyle":"-"},
    ...
    #displaying
    plt.plot(x, z_score_values, label=z_score_label, **z_score_plot_kwargs)
    plt.plot([np.min(x), np.max(x)], [z_score_limit, z_score_limit], label=limit_label, **limit_plot_kwargs)
    plt.plot([np.min(x), np.max(x)], [-z_score_limit, -z_score_limit], **limit_plot_kwargs)
    plt.legend()
