...
def plot_uncertainty_function(f, u_f, min_x, max_x, values_number,
    ...
    function_color='red', uncertainty_color='blue', function_linestyle="-", uncertainty_linestyle="-", **kwargs) :
    ...
    function_color is color of function curve[deprecated : will be remove in Alpha27]
    uncertainty_color is the color of curves that represent f ± u_f [deprecated : will be remove in Alpha27]
    function_linestyle & uncertainty_linestyle are the line style of each curve (cf Matplotlib docs for futher information) [deprecated : will be remove in Alpha27]
    """
    f_plus_u = lambda *args, **kwargs : f(*args, **function_kwargs, **kwargs) + u_f(*args, **function_kwargs, **kwargs)
    plot_2Dfunction(f_plus_u, min_x, max_x, values_number, args_before_x, args_after_x, function_label=uncertainty_label, **uncertainty_plot_kwargs, color=uncertainty_color, linestyle =uncertainty_linestyle, **kwargs)#draw f+u_f

    f_minus_u = lambda *args, **kwargs : f(*args, **function_kwargs, **kwargs) - u_f(*args, **function_kwargs, **kwargs)
    plot_2Dfunction(f_minus_u, min_x, max_x, values_number, args_before_x, args_after_x, **uncertainty_plot_kwargs, color=uncertainty_color, linestyle =uncertainty_linestyle, **kwargs)#draw f-u_f

    plot_2Dfunction(f, min_x, max_x, values_number, args_before_x, args_after_x, title=title, xlabel=xlabel, ylabel=ylabel, function_label=function_label, **function_plot_kwargs, color=function_color, linestyle =function_linestyle,  **kwargs)#draw f (this is the last function drawing else title and axes labels haven't been displayed)
