...
color="blue", linestyle ="-", **kwargs) :
    ...
    color is the line color [deprecated : will be remove in Alpha27]
    linestyle is the line style [deprecated : will be remove in Alpha27]
    You can add after keywords arguments for the function "function" [deprecated : will be remove in Alpha27]
    """
    variable_list = np.linspace(min_x, max_x, values_number)
    results_list = []
    for variable in variable_list :
        results_list.append(function(*args_before_x, variable, *args_after_x, **function_kwargs, **kwargs))
    
    #displaying
    plt.plot(variable_list, results_list, plot_kwargs, color=color, linestyle=linestyle, label=function_label)
...
colormap=cm.RdYlGn, **kwargs) :
    ...
    colormap is the colormap used for displaying [deprecated : will be remove in Alpha27]
    You can add after keywords arguments for the function "function" [deprecated : will be remove in Alpha27]
    """
    ...
    for i in range(values_y) :
        ...
        for j in range(values_x) :
            ...
            results_line.append(function(*args_before_variables, variable1, *args_between_variables, variable2, *args_after_variables, **function_kwargs, **kwargs))
    ...
    ax.plot_surface(array_x, array_y, array_z, cmap=colormap, linewidth=linewidth, **plot_kwargs)#linewidth : distance between two lines
