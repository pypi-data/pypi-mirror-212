...
class csr_ha(object):
    ...
    def set_all_routes(self, node, event_type, route_table):
        self.event_logger.info("Avani debug: route table: {}".format(route_table))
        ...
        # CDETs ID: Read disableBgpRoutePropagation property from the route table
        # and pass it in the payload
        route_propagation = route_table['properties']['disableBgpRoutePropagation']
        self.event_logger.info("Avani debug: route propagation value: {}".format(route_propagation))

        payload = "{\"location\":\"%s\", \"etag\":\"%s\", \"properties\":{\"disableBgpRoutePropagation\":\"%s\", \"routes\":[" % (
            route_table['location'], new_etag, route_propagation)
