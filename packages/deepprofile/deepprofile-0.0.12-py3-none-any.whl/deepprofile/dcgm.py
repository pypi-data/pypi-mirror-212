...
def dcgm_profiling_decorator(
    fieldIds=[1003, 1004], dcgmSamplingInterval=1000, dcgmMaxKeepAge=1000000000
):
...
if __name__ == "__main__":
    ...
    with dcgm_profiling_decorator(dcgmSamplingInterval=1000) as gm:
    ...
    print(f"Num steps: {len(metrics.values[0][1003].values)}")

    with dcgm_profiling_decorator(dcgmSamplingInterval=2000) as gm:
        start_t = timeit.default_timer()
        a, b = torch.rand(1000, 1000), torch.rand(1000, 1)
        a = a.cuda()
        b = b.cuda()
