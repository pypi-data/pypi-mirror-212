...
def prepare_command():
    ...
    def decorator(func):
        ...
        def inner(*args, **kwargs):
            ...
            cnvrg = Cnvrg()
            config = Config()
            # In case the user logged in through the sdk, the config file might not exist. congig.save will create it
            config.save()
            ...
            if "webapp" in func_args:
                try:
                    ...
                    slug = kwargs.get("slug")
                    if slug and config.data_owner_slug:
                        inject_args["webapp"] = cnvrg.projects.get(config.data_owner_slug).webapps.get(slug)
