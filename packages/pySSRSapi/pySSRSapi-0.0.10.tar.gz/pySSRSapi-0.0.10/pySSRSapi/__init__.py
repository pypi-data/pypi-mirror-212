...
class Api:
    ...
    def __patch(self, endpoint: str, data: Folder|Datasource, args: str = ''):
        response = requests.patch(self.url + endpoint + args, json=data.to_obj(), auth=self.basic_auth, headers=self.headers)
    ...
    def editFolder(self, id: str, folder: Folder):
        return self.__patch(endpoint=EP_FOLDERS, args='(' + id + ')', data=folder)
