...
# Unit16 (0-65535)
def WriteUint16_tMBus(comportname,boud,modbusnum,address,numreg,data):
	global inbuff
	...
	if SendMBus (comportname, boud, modbusnum, 0x10, address, bytearr, numreg):
		return 1
	else:
		return 0
...
# 4 байта
def WriteFourBytesMBus(comportname,boud,modbusnum,address,numbyte,data):
	global inbuff
	byte_arr=bytearray(numbyte)
	for i in range(numbyte): 
		byte_arr[i]=data[i]
	numreg=int(numbyte/2)
	if(SendMBus(comportname,boud,modbusnum,0x10,address,byte_arr,numreg)):
		return 1
	else :
		return 0

# 2 байта
def WriteTwoBytesMBus(comportname,boud,modbusnum,address,numreg,data):
	global inbuff
	byte_arr=bytearray(2)
	byte_arr[1]=data[0]
	byte_arr[0]=data[1]
	if(SendMBus(comportname,boud,modbusnum,0x10,address,byte_arr,1)):
		return 1
	else :
		return 0

# float (Число двойной точности) 8 байт
def WriteFloatMBus(comportname,boud,modbusnum,address,numfloat,data):
	global inbuff
	# global fa
	...
	if(SendMBus(comportname,boud,modbusnum,0x10,address,byte_arr,numfloat*2)):
		return 1
	else :
		return 0
...
# UShot - 0-65535
def ReadUShortMBus(comportname,boud,modbusnum,address,numreg):
	global inbuff
	global ia
	bytearr=bytearray(numreg*2)
	if(SendMBus(comportname,boud,modbusnum,3,address,0,numreg)):	
		for i in range(numreg):
...
# Uint16 - 0-65535
def ReadUint16_tMBus(comportname,boud,modbusnum,address,numreg):
	global inbuff
	global ia
	bytearr=bytearray(numreg*2)
	if(SendMBus(comportname,boud,modbusnum,3,address,0,numreg)):	
...
def ReadBytesMBus(comportname,boud,modbusnum,address,numbytes):
	global inbuff
	# global ia
...
# numreg - количество читаемых регистров
def ReadTwoBytesMBus(comportname,boud,modbusnum,address,numreg):
	global inbuff
	# global ia
...
# Чтения статуса шины?
def ReadStMBus(comportname,boud,modbusnum):
	global inbuff
	# global ia
	byte_arr=bytearray(8)
	if SendMBus(comportname,boud,modbusnum,0x07,0,0,1):
		bytearr=tuple(inbuff)
		return bytearr[2]
	else :
		return 0

def ReadInfMBus(comportname,boud,modbusnum):
	global inbuff
	# global ia
	byte_arr=bytearray(8)
	strinf=SendMBus(comportname,boud,modbusnum,0x11,0,0,1)
	if strinf:
		return strinf
	else :
		return 0

def ReadIntMBus(comportname,boud,modbusnum,address,numint):
	global inbuff
	global ia
	byte_arr=bytearray(255)
	if(SendMBus(comportname,boud,modbusnum,3,address,0,numint*2)):	
		bytearr=tuple(inbuff)
...
def ReadInt32_tMBus(comportname,boud,modbusnum,address,numint):
	global inbuff
	global ia
	byte_arr=bytearray(255)
	if(SendMBus(comportname,boud,modbusnum,3,address,0,numint*2)):	
...
def ReadRegMBus(comportname,boud,modbusnum,address,numreg):
	global inbuff
	# global ia
...
def ReadFloatMBus(comportname,boud,modbusnum,address,numfloat):
	global inbuff
	global fa
	byte_arr=bytearray(255)
	if(SendMBus(comportname,boud,modbusnum,3,address,0,numfloat*2)):	
		bytearr=tuple(inbuff)
		for i in range(numfloat):
			FloatBytes=bytearray(4)
			for j in range(4):
...
def SendMBus(comportname,boud,modbusnum,modbusfun,address,data,lengthreg):
	global inbuff
	global tuple_inbuff
	balen=bytearray(4)
	...
	if(modbusfun==0x07): #----------------read byte
		...
		sizeout=4
		sizein=5
	elif(modbusfun==0x03):	#----------------read bytes (two?)
		outbuff=bytearray(8)
		outbuff[0]=modbusnum
		outbuff[1]=3
		...
		sizein=8 
	elif(modbusfun==0x11):	#----------------read id device
		outbuff=bytearray(4)
		outbuff[0]=modbusnum
		outbuff[1]=0x11
		crc16=CalcMbusCrc16(outbuff,2)
	...
	try:
		ser=serial.Serial(comportname, boud, timeout=0.5)
	...
	ser.write(outbuff)
	if(modbusfun==0x11):
		# Читаем серийный номер устройства
		inbuff=ser.read(128)
		ser.close()
		...
		# Читаем любые данные, кроме серийного номера устройства
		inbuff=ser.read(sizein)
		ser.close()
		# print('inbuff=',inbuff)
		if(len(inbuff)==0):
			return 0
		crc16=CalcMbusCrc16(inbuff,sizein)
		if(crc16==0):
			return inbuff
		else:
			return (0)
...
def extract_val_param(str_change,par):
	# print('extract_val_param', str_change)
	str_change=str(str_change)
	len_str_change=len(str_change)
	# print('len_str_change=',len_str_change) 
