...
class SSHClient:
    ...
    def MakePath(self, remotePath):
        targetPath:str = remotePath.replace('\\', '/')
        dotPos = targetPath.rfind('.')
        lastPathPos = targetPath.rfind('/')

        if dotPos > 0 and lastPathPos < dotPos:
            targetPath = targetPath[0: lastPathPos]
        ...
        for i in range(0, subPathCount):
            ...
            print("will make path ", curCheckPath)
