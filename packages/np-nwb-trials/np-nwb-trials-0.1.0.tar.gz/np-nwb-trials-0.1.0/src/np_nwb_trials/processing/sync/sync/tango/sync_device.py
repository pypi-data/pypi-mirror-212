...
from sync import SyncEvent, SyncSampled
...
#------------------------------------------------------------------------------
    ...
    def init_device(self):
        ...
        self.attr_pulse_freq = 100000.0
        self.attr_output_path = "C:/sync/output/test.h5"
        self.attr_line_labels = "[]"
        self.attr_mode = "event"
        self.attr_delete_on_copy = False
...
#------------------------------------------------------------------------------
    ...
    def read_mode(self):
        return self.attr_mode

    def write_mode(self, data):
        self.attr_mode = data

    def read_delete_on_copy(self):
        return self.attr_delete_on_copy

    def write_delete_on_copy(self, data):
        self.attr_delete_on_copy = data
...
#------------------------------------------------------------------------------
    ...
    def start(self):
        ...
        if self.attr_mode == 'event':

            self.sync = SyncEvent(device=self.attr_device,
        ...
        elif self.attr_mode == 'freq':

            self.sync = SyncSampled(device=self.attr_device,
                                    bits=self.attr_event_bits,
                                    output_path=self.attr_output_path,
                                    freq=self.attr_pulse_freq,
                                    buffer_size=10000,
                                    verbose=True,)
            ...
            raise ValueError("Invalid sampling mode: %s" % self.attr_mode)
    ...
    def load_config(self, path):
        ...
        self.attr_mode = str(config['mode'])
