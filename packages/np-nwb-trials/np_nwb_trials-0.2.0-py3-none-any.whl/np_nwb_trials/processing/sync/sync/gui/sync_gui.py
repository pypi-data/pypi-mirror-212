...
import numpy as np
import warnings
warnings.simplefilter("ignore") #ignore hdf5 warnings
...
from sync.utils.h5 import chunk2hdf5

from sync.ni.daq import AnalogInput, System, Device, CounterInputU32
...
LAST_SESSION = os.path.join(BASE_DIR, "last.yml")
OUTPUT_DIR = os.path.join(BASE_DIR, "output")
DEFAULT_OUTPUT = os.path.join(OUTPUT_DIR, "test")
LOG_DIR = os.path.join(BASE_DIR, "logs")
CONFIG_DIR = os.path.join(BASE_DIR, "config")

for folder in [BASE_DIR, OUTPUT_DIR, LOG_DIR, CONFIG_DIR]:
...
class MyForm(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        ui_loc = os.path.join(os.path.dirname(gui_path), "sync_gui.ui")

        self.ui = uic.loadUi(ui_loc, self)
        self.ui.lineEdit_output_path.setText(DEFAULT_OUTPUT)
    ...
    def _setup_communication(self):
        ...
        try:
            from sync.zro.gui_interface import ZROInterface
        except ImportError as e:
            self.write_text("ZMQ interface failed to import.  No remote control for this session.")
            self.ui.checkBox_comm.setChecked(False)
            self.ui.checkBox_comm.setEnabled(False)
            self.disable_visualizer()
            return
        if state:
            try:
                ##TODO: let user specify ports
                self.com = ZROInterface(rep_port=REPLY_PORT,
                                        gui_handle=self)
            except Exception as e:
    ...
    def load_config(self, path=""):
        ...
        if not path:
            ...
            logging.debug("File path: {} selected".format(file_path))
            if isinstance(file_path, tuple):
                file_path = str(file_path[0])    # PyQt4 -> PySide difference
            if not file_path:
                return
        else:
            file_path = path
    ...
    def _enable_ui(self):
        ...
        if self.ui.checkBox_comm.checkState():
    ...
    def clear(self):
        ...
        if self.ci:
            ...
            try:
                ...
                logging.warning("Failed to clean up counter binary file: {}".format(e))
