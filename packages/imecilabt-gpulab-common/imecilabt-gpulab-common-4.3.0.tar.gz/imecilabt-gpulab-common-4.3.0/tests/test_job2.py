...
TEST_JOB2_REQUEST_JSON_BACKWARD_COMP = """{
    "resources": """+TEST_JOB2_REQUEST_RESOURCES_JSON+""",
    "scheduling": """+TEST_JOB2_REQUEST_SCHEDULING_JSON_BACKWARD_COMP+""",
    "docker": """+TEST_JOB2_REQUEST_DOCKER_JSON+""",
    "extra": """+TEST_JOB2_REQUEST_EXTRA_JSON+"""
}"""
...
TEST_JOB2_REQUEST_DOCKER_OBJ_A = JobRequestDocker(
    ...
    working_dir="/root/",
    storage=[
        JobStorage(container_path="/proj", host_path="/project", size_gb=None),
...
def test_job2_request_from_json_backward_comp():
    out_job2_request: JobRequest = JobRequest.from_json(TEST_JOB2_REQUEST_JSON_BACKWARD_COMP)
    assert out_job2_request == TEST_JOB2_REQUEST_OBJ_A
...
def test_job_a_to_from_dict():
    expected = TEST_JOB2_OBJ_A
    actual = Job.from_dict(TEST_JOB2_OBJ_A.to_dict())
    assert actual == expected
...
def test_jobs_from_dict_1():
    input = []
    expected = []
    actual: List[Job] = Job.from_dict_list(input)
    assert actual == expected
...
def test_job_a_to_dict_eventtimes():
    ...
    assert 'QUEUED' in actual_dict['state']['eventTimes'].keys()
    assert actual_dict['state']['eventTimes']['QUEUED'] == '2020-03-17T09:59:20Z'
...
def test_port_mapping_from_docker_dict_a():
    d = {"8888/tcp": [{"HostIp": "0.0.0.0", "HostPort": "32769"}]}
...
def test_matches_gpu_model_1():
    reqResources = JobRequestResources(cpus=1, gpus=1, cpu_memory_gb=1,
                                       gpu_model=["1080", "blah"])
...
def test_matches_gpu_model_4():
    req_resources = lambda req_model: JobRequestResources(cpus=1, gpus=1, cpu_memory_gb=1, gpu_model=[req_model])
    ...
    assert req_resources("nvidia a100 80gb pcie (80 gb mem)").matches_gpu_model("NVIDIA A100 80GB PCIe (80 GB Mem)")
    assert not req_resources(" NVIDIA A100 80gb PCIe (80 GB Mem)").matches_gpu_model("NVIDIA A100 80GB PCIe (80 GB Mem)")
    assert not req_resources("nvidia a100 80gb pcie (80 gb mem) ").matches_gpu_model("NVIDIA A100 80GB PCIe (80 GB Mem)")
...
def test_resource_request_gpu_model_dict1():
    ...
    assert req_resources == dataclasses.replace(TEST_JOB2_OBJ_A.request.resources, gpu_model=['TITAN'])
...
PERFORMANCE_TEST_SIZE = 2000  # dataclasses_json implementation of from_dict was 45.782864 s for 10000 !!!
# PERFORMANCE_TEST_MAX_MS_PER = 1.1
PERFORMANCE_TEST_MAX_MS_PER = 5.0

def performance_measure():
    dict_in = TEST_JOB2_OBJ_A.to_dict()
    for i in range(1, PERFORMANCE_TEST_SIZE):
        job_out = Job.from_dict(dict_in)
        assert TEST_JOB2_OBJ_A == job_out


def test_job2_performance():
    ...
    each_ms = (diff.total_seconds()*1000.0)/PERFORMANCE_TEST_SIZE
    print(f'performance: {diff.total_seconds()} s for {PERFORMANCE_TEST_SIZE} -> {each_ms} ms each')
    assert diff.total_seconds() < ((PERFORMANCE_TEST_MAX_MS_PER * PERFORMANCE_TEST_SIZE) / 1000.0), \
        f'bad Job.from_dict() performance: {diff.total_seconds()} s for {PERFORMANCE_TEST_SIZE} ' \
        f'-> {each_ms} ms each, which is > max {PERFORMANCE_TEST_MAX_MS_PER} ms each'
...
def test_job_auto_restart_info_a():
    ...
    assert job1.restart_info.initial_job_uuid == job1.uuid
    ...
    assert 'restartInfo' in job1.to_json()
    assert 'restartInfo' in job1.to_dict()
...
def test_job2_scheduling_reservation_ids_single_c():
    ...
    #del dict_in['reservationIds']  # keep
    actual = JobRequestScheduling.from_dict(dict_in)
    assert actual == expected
...
def test_job2_scheduling_reservation_ids_single_d():
    ...
    expected = dataclasses.replace(expected, reservation_ids=[])
...
def test_job2_scheduling_reservation_ids_single_i():
    expected = TEST_JOB2_REQUEST_SCHEDULING_OBJ_A
    dict_in = expected.to_dict()
