...
def _remove_waste_verdict(d: Dict) -> Dict:
    # We always recalculate the verdict
    if 'verdict' in d:
        del d['verdict']
    return d
...
@dataclass_dict_convert(dict_letter_case=camelcase, preprocess_from_dict=_remove_waste_verdict)
@dataclass(frozen=True)
@dataclass_auto_type_check
@dataclass_copy_method
class WasteReport:
    updated: str  # date in RFC3339 format
    waste_last_5min: WasteStat
    waste_since_start: WasteStat
    total_slot_count: int  # how many 5 minute slots were looked at?
    wasted_slot_count: int  # how many of the 5 minute slots were "wasted"?
    wasted_slot_percent: float  # how much of the total job time was "wasted"?
    final: bool  # this is the final report, as the job has finished
    verdict: WasteVerdict = field(init=False)  # is this job wasting resources, or not?

    def __post_init__(self):
        # we always recalculate the verdict
        super().__setattr__('verdict', self._calc_verdict())

    def _calc_verdict(self) -> WasteVerdict:
        # Not all jobs can have a meaningful GOOD/BAD verdict
        if self.final and self.total_slot_count <= 4:
            return WasteVerdict.SHORT
        if not self.final and self.total_slot_count <= 8:
            return WasteVerdict.UNDECIDED

        if self.wasted_slot_percent <= 10.0:
            return WasteVerdict.GOOD
        else:
            return WasteVerdict.WASTE
