...
def _get_prometheus_config():
    return resource_filename("launch", "prometheus/config.yaml")


def start_prometheus() -> str:
    container_name = f"{LAUNCHFLOW_PROM_CONTAINER_NAME}-{random.randint(0, 1000)}"
    cmd = f"docker run -p 9090:9090 --add-host host.docker.internal:host-gateway -v {_get_prometheus_config()}:/etc/prometheus/prometheus.yml -d --name {container_name} prom/prometheus"  # noqa: E501
...
def initialize_local_runtime_environment_and_block():
    ...
    ray_process = subprocess.Popen(f"{RAY_START_CMD} --block", shell=True)
