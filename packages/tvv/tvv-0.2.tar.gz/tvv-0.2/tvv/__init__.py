...
#Условие:
#Абсолютно непрерывная случайная величина X может принимать значения только в отрезке [4,9] На этом отрезке плотность распределения случайной величины X имеет вид:
#f(x)=C(1+7x^0.5+8x^0.7+4x^0.9)^1.3, где C – положительная константа. Найдите: 1) константу C 2) математическое ожидание E(X) 3) стандартное отклонение σ X
#4)квантиль уровня 0,9 распределения X
...
#равномерно распределена на отрезке [4,8]. Случайная величина Y выражается через X
#следующим образом: Y=(1+6X^0,5+4X^0,7+5X^0,9)^1,3. Найдите: 1) математическое ожидание E(Y); 2) стандартное отклонение σY; 3) асимметрию As(Y); 4) квантиль уровня 0,8 распределения Y.
...
#(X,Y)∼N(−8;16;49;1;0,8) найдите вероятность
#P((X−3)(Y−7)<0)
...
#fXY(x,y)= (18e^[-30x^2-48xy+8x-30y^2-5y-85/24]/pi)
#Найдите: 1) математическое ожидание E(X)
#2) математическое ожидание E(Y) 3) дисперсию Var(X) 4)дисперсию Var(Y) 5) ковариацию Cov(X,Y) 6) коэффициент корреляции
#ρ(X,Y)
#Менять функцию Q и матрицу C
#C = [[коэф при x^2 * 2, коэф при xy],
        [коэф при xy, коэф при y^2*2]]
...
x, y = symbols('x y')
q = -2 * (-51/2 * x ** 2 - 45*x*y - 9*x - 51/2 * y ** 2 - 7*y - 5/6)
