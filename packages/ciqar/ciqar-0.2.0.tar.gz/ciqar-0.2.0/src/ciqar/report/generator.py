...
class ReportGenerator:
    ...
    This is meant to be very generic, the concrete details of the report generation are defined by
    the selected template. The actual generation of the data needed for rendering each template file
    is delegated to specific "data generators" that are derived from `RenderInformationGenerator`.
    This class is responsible for the creation of the output files and directories, though.
    """
    ...
    def __init__(
        ...
        self.__violation_collector = LinterResultCollector(
            violations_parser,
            source_collector.get_excluded_source_files()
        )
    ...
    def generate_report(self) -> None:
        ...
        for input_file_name, render_data_type \
        in self.__template_render_metadata["render_files"].items():
    ...
    @staticmethod
    def _get_generator_class(render_data_type: RenderDataType) -> type[RenderInformationGenerator]:
        """
    ...
    def __generate_template_file(self, template_render_information: FileRenderInformation) -> None:
        """
        ...
        self.__write_rendered_file(
            template_render_information.output_file_path,
