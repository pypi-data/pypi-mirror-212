...
from ciqar.report.datagen.rules import RuleListRenderInfoGenerator, RuleDetailsRenderInfoGenerator
...
class TestReportGenerator:
    ...
    @pytest.mark.parametrize("render_data_type, expected_generator_class", [
    ...
    def test__get_generator_class(
        ...
        """
        Ensures the correct assignment of render data generators to the enum value from the template
        metadata. This is tested separately because the `_get_generator_class()` method is patched
        in other tests to inject a mocked data generator.
        """
    ...
    @pytest.mark.parametrize("render_file_names, static_file_names", [
        (  # Single file opf each type
            ["index.html.in"],
            ["static.txt"]
        ),
    ...
    def test_generate_report(
        ...
        static_file_paths = [TEMPLATE_PATH.joinpath(file_name) for file_name in static_file_names]
        ...
        template_metadata = TemplateMetadata(
            ...
            render_metadata={
                "render_files": {
                    ...
                    render_file: RenderDataType.Summary for render_file in render_file_names
                },
        ...
        # Setup and inject a mocked render info generator (which the ReportGenerator delegates to),
        ...
        generator._get_generator_class = Mock(side_effect=[  # type: ignore[method-assign]
        ...
        expected_rendered_files = (
            REPORT_OUTPUT_PATH.joinpath(file_name[:-3])
            for file_name in render_file_names
        )
