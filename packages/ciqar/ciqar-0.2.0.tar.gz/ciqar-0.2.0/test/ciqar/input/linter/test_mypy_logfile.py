...
class TestMypyLogfileParser():
    ...
    @pytest.mark.parametrize("logfile_content, expected_violations, expected_global_messages", [
        (  # Empty file
            "", [], []
        ),
...
/src/file2.py:42: error: Need type annotation for "message_type"  [var-annotated]
            """,
            [
                Violation(filename=Path("/src/file1.py"), linenumber=37,
                    message_type="error", message_id="no-untyped-def",
                    message_text="Function is missing a return type annotation",
                ),
                Violation(filename=Path("/src/file2.py"), linenumber=42,
                    message_type="error", message_id="var-annotated",
...
/src/file.py:411: error: Can use starred expression only as assignment target
            """,
            [
                Violation(filename=Path("/src/file.py"), linenumber=411,
                    message_type="error", message_id="unknown",
...
/src/file.py:42: error: Need type annotation for "message_type"  [var-annotated]
            """,
            [
                Violation(filename=Path("/src/file.py"), linenumber=42,
                    message_type="error", message_id="var-annotated",
                    message_text="Need type annotation for \"message_type\"",
                ),
            ],
...
/path/to/file.py:64: note: (Skipping most remaining errors due to unresolved imports or missing stubs; fix these first)
            ...
            [
                ...
                Violation(filename=Path("/src/file2.py"), linenumber=51,
                    message_type="note", message_id="unknown",
...
/src/file1.py:36: error: Incompatible types in assignment (expression has type "Callable[[Any, VarArg(Any), KwArg(Any)], Any]", variable has type "Callable[[BaseType, Any], Any]")  [assignment]
            """,
            [
                Violation(filename=Path("/src/file1.py"), linenumber=36,
                    message_type="error", message_id="assignment",
...
/src/file2.py:40: error: Returning Any from function declared to return "OrdinaryUser"  [no-any-return]
            """,
            [
                Violation(filename=Path("/src/file2.py"), linenumber=40,
                    message_type="error", message_id="call-overload",
                    message_text=(
                ...
                Violation(filename=Path("/src/file2.py"), linenumber=40,
                    message_type="error", message_id="no-any-return",
...
/usr/lib/python3.7/site-packages/sqlalchemy-stubs/sql/schema.pyi:314: note: "MetaData" defined here
           """,
           [
                Violation(filename=Path("/src/file3.py"), linenumber=123,
                    message_type="error", message_id="call-arg",
                    message_text=(
