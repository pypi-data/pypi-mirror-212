...
class TestSourceFileCollector:
    ...
    @pytest.mark.parametrize("search_paths, init_context, expected_base_path", [
        (["/project/src"], nullcontext(), "/project/src"),
        (
            ["/project/src/lib1", "/project/src/lib2"],
            nullcontext(),
            "/project/src"
        ),
        (
    ...
    @pytest.mark.parametrize("existing_files, search_paths, exclude_paths, found_files, excluded_files", [
        (  # Single source dir
        ...
        (  # Exclude multiple paths (file & dir)
            ["/project/src/main.py", "/project/src/test_main.py", "/project/tasks.py", "/project/main.py"],
            ["/project"],
    ...
    def test_collect_source_files(
        ...
        assert sorted([str(file_info.absolute_path) for file_info in found_source_files]) == sorted(found_files)
        assert sorted([str(file_path) for file_path in excluded_source_files]) == sorted(excluded_files)

    def test_collect_source_files_once(self) -> None:
        """
        Ensures that the file system is only traversed once, i.e. a second call must use the internal cache.
        """
    ...
    @pytest.mark.parametrize("request_filepath, raise_context", [
