...
class HTTP(Response):
    def formatUrl(self, args):
        if None in args:
            raise KubefacetsAPIException("Invalid url paths passed for forming url", "url_error", status.HTTP_400_BAD_REQUEST, "Probably config error, plz check if services are up?")
        return "/".join(args)
