...
class Command(BaseCommand):
    ...
    def update_device_ss_data(self, serial_number, pid, ss_data):
        ...
        try:
            ...
            self.stdout.write(
                self.task_error(text=f"Get data for serial number {serial_number} with PID {pid}", changed=False)
            )
        ...
        no_duplicates = []
        for idx, suggestion in enumerate(ss_data["productList"]["suggestions"]):
            idx = idx + 1
        ...
        self.task_error(text=f"Get data for serial number {serial_number} with PID {pid}", changed=False)
        self.stdout.write(f"{serial_number} - recommended_release: '<Recommended software release>'")
    ...
    def update_device_type_eox_data(self, pid, eox_data):
        ...
        if "EOXError" in eox_data:
            if eox_data["EOXRecord"][0]["EOXError"]["ErrorID"] in eox_errors:
                eox_has_error = True
            # Error SSA_ERR_026 is good: EOX information does not exist for the following product ID(s)
            elif "SSA_ERR_026" in eox_data["EOXRecord"][0]["EOXError"]["ErrorID"]:
                eox_has_error = False
            else:
                eox_has_error = True
