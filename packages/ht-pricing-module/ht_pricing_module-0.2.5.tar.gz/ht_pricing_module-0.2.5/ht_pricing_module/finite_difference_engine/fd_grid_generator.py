...
class FullyImplicitEu(FiniteDifference):
    ...
    def _set_matrix_(self):
        self.A = sp.diags([self.l[1:], self.c, self.u[:-1]], [-1, 0, 1], format='csc').toarray()
        self.I = sp.eye(self.Ns - 1)
        self.M = self.I - self.dt * self.A
    ...
    def _solve_(self):
        ...
        for j in reversed(np.arange(self.Nt)):
            ...
            self.grid[1: -1, j] = np.dot(M_inv, U)
...
class CrankNicolsonEu(FiniteDifference):
    ...
    def _solve_(self):
        ...
        for j in reversed(np.arange(self.Nt)):
            ...
            U[0] += self.theta * self.l[0] * self.dt * self.grid[0, j] + (1 - self.theta) * self.l[0] * self.dt * self.grid[0, j + 1]
            U[-1] += self.theta * self.u[-1] * self.dt * self.grid[-1, j] + (1 - self.theta) * self.u[-1] * self.dt * self.grid[-1, j + 1]
