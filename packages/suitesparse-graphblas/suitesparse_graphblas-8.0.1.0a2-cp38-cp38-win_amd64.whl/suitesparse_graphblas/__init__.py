...
def _delvewheel_init_patch_1_3_7():
    ...
    if sys.version_info[:2] >= (3, 8) and not is_conda_cpython or sys.version_info[:2] >= (3, 10):
        ...
        load_order_filepath = os.path.join(libs_dir, '.load-order-suitesparse_graphblas-8.0.1.0a2')
        if os.path.isfile(load_order_filepath):
            with open(os.path.join(libs_dir, '.load-order-suitesparse_graphblas-8.0.1.0a2')) as file:
