    ...
    def update_training_counters(self, current_labels):
        if self.test_type in ['classification', 'emotives_value']:
            ...
            for emo, val in current_labels.items():
                ...
                if emotive_sign == 1:
                    emotive_label_types = ['positive', 'overall']
                elif emotive_sign == -1:
                    emotive_label_types = ['negative', 'overall']
                for emotive_label_type in emotive_label_types:
                    emo_update_dict = {emotive_label_type: 1}
                    self.labels_counter[emo].update(emo_update_dict)
                    self.training_counter[emo].update(emo_update_dict)
                    self.overall_labels_counter[emo].update(emo_update_dict)
                    self.overall_training_counter[emo].update(emo_update_dict)
        ...
        if self.test_type in ['classification', 'emotives_value']:
            ...
            for emo, val in current_labels.items():
                ...
                for emotive_label_type in emotive_label_types:
                    emo_update_dict = {emotive_label_type: 1}
                    self.labels_counter[emo].update(emo_update_dict)
                    self.testing_counter[emo].update(emo_update_dict)
                    self.overall_labels_counter[emo].update(emo_update_dict)
                    self.overall_testing_counter[emo].update(emo_update_dict)
...
def get_emotives_polarity_metrics(emotives_set, this_test_log, overall=False):
    ...
    template_dict = {'predictions': [],
                     'actuals': [],
                     'metrics': {'overall': {'true_positive': 0.0,
                                 'false_positive': 0.0,
                                             'true_negative': 0.0,
                                             'false_negative': 0.0,
    ...
    for node, results in emotive_polarity_results.items():
        for emotive, metrics in results.items():
            for emo_type in ['positive', 'negative', 'overall']:
                metrics['metrics'][emo_type]['training_counter'] = raw_test_results['training_counter'][emotive][emo_type]
                metrics['metrics'][emo_type]['testing_counter'] = raw_test_results['testing_counter'][emotive][emo_type]
                metrics['metrics'][emo_type]['counter'] = raw_test_results['counter'][emotive][emo_type]
        ...
        for emotive, val in metrics['actual'].items():
            for node, node_emotive_set in emotives_set.items():
                if emotive in node_emotive_set:
                    emotive_polarity_results[node][emotive]['actuals'].append(val)
        for node, emotive_dict in metrics['predicted'].items():
            for emotive, val in emotive_dict.items():
                emotive_polarity_results[node][emotive]['predictions'].append(val)
            ...
            for emotive in test_step_info['actual'].keys():
                ...
                if emotive in test_step_info['predicted'][k].keys():
                    ...
                    for val_type in [actual_value_type, 'overall']:
                        ...
                        test_step_info[val_type]['precision'][k][emotive] = update_precision(tp=test_step_info[val_type]['true_positive'][k][emotive],
                                                                                             tn=test_step_info[val_type]['true_negative'][k][emotive],
                                                                                             response_count=test_step_info[val_type]['response_counts'][k][emotive])

                        test_step_info[val_type]['accuracy'][k][emotive] = update_accuracy(tp=test_step_info[val_type]['true_positive'][k][emotive],
                                                                                           tn=test_step_info[val_type]['true_negative'][k][emotive],
