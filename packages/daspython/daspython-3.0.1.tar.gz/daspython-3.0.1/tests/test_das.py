...
class TestDasPython(unittest.TestCase):
    ...
    def __init__(self, methodName: str = "runTest") -> None:
        ...
        self.__das = DasClient(self.__username, self.__password,self.__base_url,self.__check_https)

    def test_get_attribute_name(self):
            unittest.TestCase.assertEqual(self, self.__das.get_attribute_name(55), 'Core')  

    def test_get_attribute_id(self):        
        unittest.TestCase.assertEqual(self, self.__das.get_attribute_id('Core'), 55)                  

    def test_get_entries(self):         
        response = self.__das.get_entries('Core')                
        unittest.TestCase.assertEqual(self, len(response.items), 10)   

    def test_download_template(self):         
        self.__das.download_template('Core', "c:\\temp\\template.xlsx")               
    ...
    def test_insert_data_from_excel(self):
        ...
        self.__das.create_entries_from_excel(attribute_name='Core', entries=entries)                
    ...
    def test_create_entry(self):
        ...
        new_entry_id = self.__das.create_entry(attribute_name='cruise', entry=entry)

        self.assertTrue(new_entry_id is not None and new_entry_id != 'Unable to create entry')
    ...
    def test_upload_digital_object(self):
        if not os.path.exists('C:\\Temp\\TEST-01.txt'):
            with open('C:\\Temp\\TEST-01.txt', 'w') as f:
                f.write('Hello World')        
        self.__das.upload_digital_object('zb.b.9w', 'C:\\Temp\\TEST-01.txt', 'Uploaded from Python')   
        response = self.__das.get_by_code('zb.b.9w')            
        digital_objects = json.loads(str(response.entry.get('6')))
        self.assertTrue(len(digital_objects) > 0 and any(obj.get('name') == "TEST-01.txt" for obj in digital_objects)) 

    def test_link_existing_digital_object(self):
        self.__das.link_digital_object('zb.b.9w','h.b.ql2h')
        response = self.__das.get_by_code('zb.b.9w')            
        digital_objects = json.loads(str(response.entry.get('6')))
        self.assertTrue(len(digital_objects) > 0 and any(obj.get('code') == "h.b.ql2h" for obj in digital_objects)) 

    def test_request_digital_objects(self):
        response = self.__das.request_digital_objects('zb.b.9w')
    ...
    def test_get_files_from_request(self):
        ...
        files = [f for f in os.listdir('.') if os.path.isfile(f) and f.startswith('digital-objects-') and f.endswith('.zip')]
    ...
    def test_search(self):
        response = self.__das.search('core', 'End Depth(75);Name(64PE407-03PC#08)')
        self.assertTrue(response is not None and len(response.items) > 0 and response.total > 0)
