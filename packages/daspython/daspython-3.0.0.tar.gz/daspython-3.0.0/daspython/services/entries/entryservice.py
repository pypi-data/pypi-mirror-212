...
class EntryService(ApiMethods):
    ...
    def insert_from_excel(self, attribute_id, entries):
        ...
        response = graphQL_server.get_attribute_excel_info(attribute['result']['alias'])
        if (response is None):
            raise Exception(f'Attribute with id: {attribute_id} not found.')
        ...
        for entry in request['entries']:  
            ...
            for key in entry:
                field = next(filter(lambda x: x.get('displayName') == key, entry_fields), None)
                if (field is None):
                    continue
                ...
                if (not field.get('customData') is None):
                    ...
                    if (not custom_data.get('datasource') is None and not custom_data.get('datasource').get('attributeId') is None):
                        parent_entry = self.get_entry(entry[key], custom_data.get('datasource').get('attributeId'))
    ...
    def __convertToInputRequest(self, body: InsertRequest) -> InsertRequest:
        ...
        if not grouped_field is None:
            custom_data = json.loads(grouped_field.get('customData'))
            if not custom_data is None:
                for key in body.entry:
                    field = next(filter(lambda x: x.get('displayName') == key, custom_data.get('fields')), None)
                    if (field is None):
                        continue                    
                    request.entry[field.get('column')] = body.entry[field.get('displayName')]                        
