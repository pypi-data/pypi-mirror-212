...
class JsonLDConverter:
    ...
    def __handle_csvw_dataset(self, lol: List[List], context: dict) -> dict:
        ...
        column_nodes = [{'@type': [f'{self.CLASS_PREFIX}Column'], 'titles': [c], 'number': i, '@id': self.get_blank_node_id()} for i, c in enumerate(columns, start=1)]
        table_node['columns'] = column_nodes
        # iterate over rows and create them
        row_nodes = []
        for i, r in enumerate(lol[1:], start=1):
            row = {'@type': [f'{self.CLASS_PREFIX}Row'], 'titles': [f'Row {i}'], 'cells': [], 'number': i}
            # run through cells
            for idx, c in enumerate(r):
                row['cells'].append({'@type': [f'{self.CLASS_PREFIX}Cell'], 'value': str(c) if not pd.isna(c) else None, 'column': column_nodes[idx]['@id']})
            # append row to rows
        ...
        # context['column'] = {"@id": f'{self.PREDICATE_PREFIX}CSVW_Column', "@type": "@id"}
        table_node['rows'] = row_nodes
        return table_node

    def get_blank_node_id(self):
        """
    ...
    def convert_2_jsonld(self, orkg_json: dict):
        """
    ...
    def handle_values(self, orkg_json: dict, jsonld: dict, context: dict):
        """
            ...
            for entity in entities:
                ...
                if 'label' in entity:
                    ...
                    if 'classes' in entity:
                        jsonld_entity['@type'] = [self.CLASS_PREFIX + c for c in entity['classes']]
...
if __name__ == '__main__':
    ...
    sample = """{"resource":{"name":"Statistically significant hypothesis test with IRE binding dependent variable on failing and non-failing hearts","classes":["C1007"],"values":{"P27011":[{"label":"http://purl.obolibrary.org/obo/GO_0030350","classes":["URI"]}],"P4003":[{"classes":["C1003"],"values":{"P9003":[{"classes":["C1004"],"values":{"P9004":[{"text":"1.3111247517411605e-08"}]},"label":""}]},"label":"the p-value of the statistical hypothesis test"}],"P4015":[{"label":"data.csv","classes":["C21033"]}]}}}"""
    print(JsonLDConverter("https://orkg.org/").convert_2_jsonld(json.loads(sample)))
