...
class CommonDataType(ABC):
    ...
    def type_name(self) -> str:
        ret = F"{get_common_data_type_from(self.TAG).NAME}"
        if isinstance(self, (Structure, Enum)) or (isinstance(self, _String) and self.SIZE is not None):
            ret += F"[{len(self)}]"
        elif isinstance(self, Digital) and self.VALUE is not None:
            ret += F"({self.VALUE})"
        return ret
...
def get_type_name(value: CommonDataType) -> str:
    ret = F"{__types[value.TAG[:1]].NAME}"
    if isinstance(value, Structure) or (isinstance(value, _String) and value.SIZE is not None):
        ret += F"[{len(value)}]"
    elif isinstance(value, Digital) and value.VALUE is not None:
        ret += F"({value.VALUE})"
    return ret
