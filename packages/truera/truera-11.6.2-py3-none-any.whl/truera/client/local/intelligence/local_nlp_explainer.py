...
class LocalNLPExplainer(NonTabularExplainer):
    ...
    def segments_widget(self, n_records=None):
        ...
        from truera.nlp.general.aiq.visualizations import SegmentsWidget

        n_records = n_records or self._attr_config.n_metrics_records

        obj = SegmentsWidget(explainer=self, n_records=n_records)
    ...
    def widgets_widget(self, token_type: str = "word"):
        ...
        tabs = [
            (
                "drivers summary", lambda: self.
                binary_driver_summary_widget(token_type=token_type)
            ),
            (
                "local drivers", lambda: self.
                record_explanations_attribution_tab(token_type=token_type)
            ),
            (
                "global drivers",
                lambda: self.global_token_summary(token_type=token_type)
            ),
            (
                "data exploration",
                lambda: self.data_exploration_tab(token_type=token_type)
            ), ("instance search", lambda: self.binary_search_widget()),
            (
                "token confusion",
                lambda: self.token_confusion_widget(token_type=token_type)
            )
        ]
        ...
        for i, t in enumerate(tabs):
            out = widgets.Output()
            but = widgets.Button(description="Load this widget")
            ...
            def pres_load(b):
                ...
                try:
                    ...
                    with info:
                        # In case the selected widget produces output instead of
                        # returning a widget. This will capture it. The "About
                        # this widget" tabs are of this form.
                        res = b.t[1]()

                    b.out.clear_output(wait=True)
                    with b.out:
                        display(res)
        ...
        return widgets.VBox([info, w])

    def token_confusion_widget(
        self, n_records: int = None, token_type: str = "word"
    ):
        """
        ...
        return viz.token_confusion_widget(
            self, n_records=n_records, token_type=token_type
        )
