...
class Tracepoint(object):
    ...
    output_logs = True  # If set, anything logged will be printed out.
...
class State(collections.abc.Iterable):
    ...
    def set(
        ...
        #if newval:
        # print(f"✅ {var}={str(val)[0:16]}\n\t", end='')
        #Tracepoint.log(trace)
        #print(Tracepoint.str_trace(trace, split="\n\t"))
    ...
    def eval(self, state: State, trace) -> State:
        ...
        for param in self.antecedents:
            if param not in state:
                raise ValueError(
                    f"cannot evaluate function {self} without {param}"
                )
...
class Rules:
    ...
    def eval_rule(self, state: State, rule: Rule, trace=[]) -> Iterable[State]:
        ...
        for branch in branches:
            ...
            branch_state = rule.eval(branch, trace=new_trace
                                     # + sats
                                    )
            yield branch_state
    ...
    def get(
        self, state: State, need: List[Parameter], optional: List[Parameter],
        trace
    ) -> Iterable[State]:
        """
        ...
        for (branch, trace) in first_branches:
            ...
            if len(rest) == 0:
                ...
                for state in self.get(
                    state=branch, need=rest, optional=optional, trace=trace2
                ):
                    yielded_something = True
                    yield state
