...
if t.TYPE_CHECKING:
    ...
    ListAny = list[t.Any]
    AnnotatedList = t.List[t.Any]
    TupleAny = tuple[t.Any, ...]
    AnnotatedTuple = t.Tuple[t.Any, ...]
    SetAny = set[t.Any]
    AnnotatedSet = t.Set[t.Any]
    DictStrAny = dict[str, t.Any]
    AnnotatedDict = t.Dict[str, t.Any]
    AnnotatedSequence = t.Sequence[t.Any]
else:
    ListAny = list
    AnnotatedList = t.List
    TupleAny = tuple
    AnnotatedTuple = t.Tuple
    SetAny = set
    AnnotatedSet = t.Set
    DictStrAny = dict
    AnnotatedDict = t.Dict
...
def parse_list_type(typ: ListAny) -> click.ParamType:
    ...


def parse_set_type(typ: SetAny) -> click.ParamType:
    ...


def parse_tuple_type(typ: TupleAny) -> click.ParamType:
    ...


def parse_dict_type(typ: DictStrAny) -> click.ParamType:
    ...


def parse_sequence_type(typ: t.Sequence[t.Any]) -> click.ParamType:
    ...


def parse_bytes_type(_: bytes) -> click.ParamType:
    return click.STRING


def parse_none_type(_: type[None]) -> None:
    return None


BUILTINS_CLICK_MAPPINGS: dict[t.Any, tuple[type[t.Any] | None, t.Callable[[t.Any], click.ParamType | None]]] = {
    AnnotatedList: (ListAny, parse_list_type),
    AnnotatedSet: (SetAny, parse_set_type),
    AnnotatedDict: (DictStrAny, parse_dict_type),
    AnnotatedTuple: (TupleAny, parse_tuple_type),
    t.ByteString: (bytes, parse_bytes_type),  # https://docs.python.org/3/library/typing.html#typing.ByteString
    AnnotatedSequence: (collections.abc.Sequence, parse_sequence_type),
    type(None): (None, parse_none_type),
}
...
class UnionType(click.ParamType):
    ...
    def __init__(self, hints: type) -> None:
        self.hints = hints
