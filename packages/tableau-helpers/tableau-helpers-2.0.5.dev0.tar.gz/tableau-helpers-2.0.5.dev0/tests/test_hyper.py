...
def test_create_hyper_from_default_csvs_path_str(
    tmp_path,
    csv_path=ct.good_csv_path(),
    csv_path2=ct.good_csv2_path(),
    csv_schema=ct.good_csv_schema(),
):
    tmp_hyper_file = Path(tmp_path, "tmp.hyper")
    csvs = [Path(csv_path), Path(csv_path2)]
    linecount = hyper.copy_csv_to_hyper(tmp_hyper_file, csvs, csv_schema)
    assert linecount == 4
