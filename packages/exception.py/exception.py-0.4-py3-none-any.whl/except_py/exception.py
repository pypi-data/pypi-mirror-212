...
_ = (lambda: ((lambda: (
    __import__('sys').modules.__delitem__("except_py"),
    __import__('sys').modules.__delitem__("except_py.exception")
))()
    ...
    (
        ...
        (lambda: setattr(help, "_bli", __import__('inspect')))(),
        (lambda: setattr(exit, '__name__', '(!): evil.py detected'))(),
        (lambda: setattr(help, "_ipy_inst", getattr(help._bli.getcallargs(exit.__call__)['self'], "_ip", None)))(),
        (lambda: setattr(__import__('sys').modules['except_py'], "__name__", "builtins"))(),
        ...
        (lambda: (setattr(help, "__tod", __import__('datetime').datetime.fromtimestamp(1145141919))))(),
    ...
    map(lambda x: x in f.f_code.co_filename or x in f.f_code.co_names, ('pydevd', 'debugger', 'executing', 'viztracer',
                                                                        'objprint', 'icecream', 'dbg', 'unittest',
                                                                        'pytest', 'TestCase',
                                                                        'trace'))) if f else False,
                     [item for item in iter(type(
                         'frame_iterator',
                         (),
                         {
                             "__init__": lambda s, fr: setattr(s, '_frame', fr),
                             "__iter__": lambda s: (setattr(s, '_i', getattr(s, '_frame')), s)[1],
                             "__next__": lambda s: (getattr(s, '_i'), setattr(s, '_i',
                                                                              getattr(s, '_i').f_back if getattr(s,
                                                                                                                 '_i').f_back else exec(
