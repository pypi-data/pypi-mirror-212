...
def dataset(run_id, data, labels, name):
  data = np.array(data)
  ...
  np.save('mynacode/'+str(run_id)+'/'+str(name)+'_data.npy', data)
  np.save('mynacode/'+str(run_id)+'/'+str(name)+'_labels.npy', labels)
...
def save_pytorch_model(run_id, network, state_dict_path):
    ...
    files = {'network': open('mynacode/'+str(run_id)+'/saved_network.pkl','rb'), 'state_dict': open(state_dict_path,'rb')}
...
def recreate_pytorch_results(run_id, threshold=0.5, test_data=None, test_labels=None):
  ...
  if test_data == None:
    test_set = np.load('mynacode/'+str(run_id)+'/'+'test_data.npy')
    predictions = net(torch.from_numpy(test_set).float())
