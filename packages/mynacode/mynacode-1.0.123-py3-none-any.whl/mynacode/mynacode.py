...
def pytorch_dataset(run_id, dataset, name='train'):
  ...
  dataset_dict = {str(name)+'_shape' : str(train.shape), str(name)+'_mean': np.mean(train), str(name)+'_median': np.median(train),
                str(name)+'_min': np.min(train), str(name)+'_max': np.max(train), str(name)+'_zeros': np.count_nonzero(ytrain==0),
                str(name)+'_ones': np.count_nonzero(ytrain==1)}

  data = {'run_id': run_id, 'dataset': str(dataset_dict), 'name': name, 'username': username, 'key': key}

  response = requests.post(protocol+'://'+IP+'/api/add_pytorch_dataset', data=data)
  if response.text == '0':
    print("Authentication failed")
