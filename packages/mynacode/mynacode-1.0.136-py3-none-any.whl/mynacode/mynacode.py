...
def save_torch_dataloader(run_id, dataloader, name):
  ...
  with open('mynacode/'+str(run_id)+'/'+name+'_dataloader.pkl', 'wb') as f:
...
def load_torch_dataloader(run_id, name):
  with open('mynacode/'+str(run_id)+'/'+name+'_dataloader.pkl', 'rb') as f:
      dataloader = dill.load(f)

  return dataloader


def recreate_pytorch_results(run_id, threshold=0.5, test_dataloader=None):
  ...
  dataloader = load_torch_dataloader(run_id, 'test')
