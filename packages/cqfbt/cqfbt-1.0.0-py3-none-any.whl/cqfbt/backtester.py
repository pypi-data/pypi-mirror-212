...
class Engine():
    ...
    def add_data(self, path):
        ...
        timestamp_range = pl.date_range(low=data['Date'][0], high=data['Date'][data.shape[0]-1], interval=str(
            self.date_interval[1])+str.lower(self.date_interval[0]))
    ...
    def reformat_arr(self):
        ...
        if self.market_arr.is_empty():
            ...
            data = pl.read_csv(
                "cqfbt/data/" + "market_benchmark_internal_hist.csv")
            try:
                data = data.rename({'Datetime': 'Date'})
            except:
                pass
    ...
    def set_transaction_costs(self, transaction_costs):
        ...
        if isinstance(transaction_costs, float):
            self.transaction_costs = [
                transaction_costs] * len(self.portfolio_assets)
        # if its a float list
        elif isinstance(transaction_costs, list):
            # make sure the list is the same length
            assert len(transaction_costs) == len(
                self.portfolio_assets), 'Length of transaction_costs must match the number of assets'
            self.transaction_costs = transaction_costs
        else:
            raise TypeError(
                'Transaction costs must be of type float or list of floats')
    ...
    def performance_plot(self):
        ...
        for j in range(len(self.strategies)):
            ...
            values = np.add(self.portfolio_history[j][:, len(
                self.portfolio_assets)+1], self.portfolio_history[j][:, len(self.portfolio_assets)])
            performance.xaxis.set_major_locator(
                MaxNLocator(integer=True, prune='both', nbins=9))
            ...
            allocs = self.portfolio_history[j][:, :len(
                self.portfolio_allocations[j])]
            ...
            for i in range(len(self.arr)):
                ...
                yearly_returns = ((end_price - start_price) /
                                  start_price) ** (1/years) * 100

                time_diff_days = (
                    self.dates[1] - self.dates[0]).total_seconds()*len(self.dates) / 86400
                ...
                hsv = cm.get_cmap('hsv')
                scatter_plot.scatter(volatility.real, yearly_returns.real, marker='o', s=mean_price *
                                     alloc_sums[i]/max(alloc_sums), color=hsv(i/len(self.portfolio_assets)), alpha=0.6)
                scatter_plot.text(volatility.real, yearly_returns.real,
                                  self.portfolio_assets[i], ha='center', va='bottom', fontsize=8)
                ...
                if(len(self.dates) > 400):
                    ...
                    num_values_to_pad = num_vals_per_bar - \
                        len(alloc_values[:, i, 0]) % num_vals_per_bar

                    padded_alloc_vals = np.pad(
                        alloc_values[:, i, 0], (0, num_values_to_pad), mode='constant')
                    padded_alloc_value_sums = np.pad(
                        alloc_value_sums[:, 0], (0, num_values_to_pad), mode='constant')

                    groups = padded_alloc_vals.reshape(-1, num_vals_per_bar)
                    group_sums = padded_alloc_value_sums.reshape(
                        -1, num_vals_per_bar)

                    group_avg = np.sum(groups, axis=1) / num_vals_per_bar
                    group_avg_sum = np.sum(
                        group_sums, axis=1) / num_vals_per_bar
                    new_dates = []
                    for k in range(0, len(self.dates), num_vals_per_bar):
                        new_dates.append(self.dates[k])

                alloc_plot.bar(new_dates[:], group_avg[:], 1, bottom=group_avg_sum[:], align='center', label=self.portfolio_assets[i], color=hsv(
                    i/len(self.portfolio_assets)), edgecolor='black', linewidth=0.1)
    ...
    def get_treynor_ratio(self) -> list[float]:
        # We use the 3-month treasury bill rate as of 4/18/23 as a proxy for the risk-free ratio
        RISK_FREE_RATE = .0504

        treynor_ratio = []
        for i in range(len(self.strategies)):
            portfolio_history = self.portfolio_history[i]
            portfolio_value = np.add(portfolio_history[:, len(
                self.portfolio_assets)], portfolio_history[:, len(self.portfolio_assets) + 1])
            strategy_returns = np.divide(
                np.diff(portfolio_value, axis=0), portfolio_value[:-1])
    ...
    def get_value_at_risk(self) -> list[float]:
        ...
        for i in range(len(self.strategies)):
            ...
            weights = self.portfolio_allocations / \
                np.sum(self.portfolio_allocations)
            port_mean = np.dot(avg_returns, weights)
            port_stdev = np.sqrt(
                np.dot(np.dot(weights.T, cov_matrix), weights)),
            mean_inv = (1 + port_mean) * self.strategy_capital[i]
            stdev_inv = self.strategy_capital[i] * port_stdev

            # select confidence interval of 95%
            cutoff = norm.ppf(0.05, mean_inv, stdev_inv)
            VaRs.append(self.strategy_capital[i] - cutoff)
        return VaRs
    ...
    def get_conditional_value_at_risk(self) -> list[float]:
        ...
        for i in range(len(self.strategies)):
            ...
            returns = np.divide(
                np.diff(portfolio_value, axis=0), portfolio_value[:-1])
            var_pct_loss = VaRs[i] / self.strategy_capital[i]
            cvars.append(
                self.strategy_capital[i] * np.nanmean(returns[returns < var_pct_loss]))
        return cvars
    ...
    def get_burke_ratio(self) -> list[float]:
        ...
        for i in range(len(self.strategies)):
            ...
            returns = np.divide(
                np.diff(portfolio_value, axis=0), portfolio_value[:-1])
            burke_ratio = (returns - RISK_FREE_RATE) / \
                np.sqrt(np.sum(np.square(drawdowns)))
    ...
    def get_m2_ratio(self) -> list[float]:
        ...
        sharpes = self.get_sharpe_ratio()
        m2s = sharpes * benchmark + RISK_FREE_RATE
        return m2s
