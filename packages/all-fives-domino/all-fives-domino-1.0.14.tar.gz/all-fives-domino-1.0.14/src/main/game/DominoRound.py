...
class DominoRound:
    def __init__(self, player1: "Player", player2: "Player", begins: Union["Player", None] = None, target_score: int = 305):
    ...
    def award_score(self):
        ...
        # If the player wins due to this, end the game
        if self.current_player.score >= self.target_score:
            loser = self.player1 if self.current_player == self.player2 else self.player1
            raise GameWinSignal(self.current_player, loser)
