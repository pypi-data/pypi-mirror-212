...
def get_snowflake_dataframe(
    ...
    if table and query:
        msg = f"Should only specify one of table and query sources for Snowflake"
        raise AssertionError(msg)
    if not table and not query:
        msg = f"Missing both table and query sources for Snowflake, exactly one must be present"
        raise AssertionError(msg)
