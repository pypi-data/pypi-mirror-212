...
class TestFunction(object):
    def __init__(self, dim, name="TestFunction", x_scaler=None, y_scaler=None):
        self._name = name
        self._dim = dim
    ...
    def _cal_descaled(a_scaled, a_scaler):
        if isinstance(a_scaled, np.ndarray) and len(a_scaled.shape) == 2:
            return (
                a_scaled / np.tile(a_scaler[1], (a_scaled.shape[1], 1)).transpose()
            ) - np.tile(a_scaler[0], (a_scaled.shape[1], 1)).transpose()
        elif isinstance(a_scaled, torch.Tensor) and len(a_scaled.shape) == 2:
            return (
                a_scaled / torch.tile(a_scaler[1], (a_scaled.shape[1], 1)).T
            ) - torch.tile(a_scaler[0], (a_scaled.shape[1], 1)).T
    ...
    def auto_init_scaler(self, auto_init_x_scaler=True, auto_init_y_scaler=True):
    ...
    def to_torch(self):
        self._if_torch = True
        self._min_pt = torch.tensor(self._min_pt)
        self._x_bound = torch.tensor(self._x_bound)
...
class BOTorchTestFunction(TestFunction):
    def __init__(self, func, name="BOTorchTestFunction", x_scaler=None, y_scaler=None):
        self.botorch_func = func
        super().__init__(self.botorch_func.dim, name, x_scaler, y_scaler)
    ...
    def auto_init_scaler(self, auto_init_x_scaler=True, auto_init_y_scaler=True):
        super().auto_init_scaler(auto_init_x_scaler, auto_init_y_scaler)
