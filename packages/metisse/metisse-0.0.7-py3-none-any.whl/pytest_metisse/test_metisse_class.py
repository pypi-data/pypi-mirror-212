...
def test_metisse_class_init_android(test_metisse_class_test_dir):
    ...
    with patch("metisse.utils.metisse_log.MetisseLogger.__init__", return_value=None)as mock_metisse_log_init:
        metisse = MetisseClass(device_id, relatively_path, pyqt6_ui_label, os_environment)
...
def test_metisse_class_init_ios(test_metisse_class_test_dir):
    ...
    with patch("metisse.utils.metisse_log.MetisseLogger.__init__", return_value=None)as             mock_metisse_log_init , \
        patch("metisse.clients.ios.wda.WdaClient.__init__", return_value=None) as mock_wda_client_init:
        metisse_ios = MetisseClass(device_id, relatively_path, pyqt6_ui_label, os_environment_ios)
...
def test_metisse_class_init_invalid_os(test_metisse_class_test_dir):
    ...
    with patch("metisse.utils.metisse_log.MetisseLogger.__init__", return_value=None)as mock_metisse_log_init:
        with pytest.raises(AssertionError):
            MetisseClass(device_id, relatively_path, pyqt6_ui_label, os_environment_invalid)
            mock_metisse_log_init.assert_called_once()
