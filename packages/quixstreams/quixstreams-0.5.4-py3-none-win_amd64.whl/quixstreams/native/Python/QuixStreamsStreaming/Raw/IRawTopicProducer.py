...
class IRawTopicProducer(object):
    ...
    def add_OnDisposed(self, value: Callable[[c_void_p, c_void_p], None]) -> None:
        ...
        InteropUtils.invoke("irawtopicproducer_add_ondisposed", self._pointer, value_func_wrapper_addr)
    ...
    def remove_OnDisposed(self, value: Callable[[c_void_p, c_void_p], None]) -> None:
        ...
        if value is not None:
            ...
            value_func_wrapper_addr = ctypes.cast(value_converter_func_wrapper, c_void_p)
            if InteropUtils.DebugEnabled:
                print("Registering value_converter_func_wrapper in irawtopicproducer_remove_ondisposed, addr {}".format(value_func_wrapper_addr))
                value_func_wrapper_addr_val = value_func_wrapper_addr.value
                # TODO IRawTopicProducer._weakrefs.append(weakref.ref(value_converter_func_wrapper, lambda x: print("De-referenced value_converter_func_wrapper in irawtopicproducer_remove_ondisposed, addr {}".format(value_func_wrapper_addr_val))))
        
        InteropUtils.invoke("irawtopicproducer_remove_ondisposed", self._pointer, value_func_wrapper_addr)
        return value_func_wrapper_addr
