...
class StreamPropertiesConsumer(object):
    ...
    def add_OnChanged(self, value: Callable[[c_void_p, c_void_p], None]) -> None:
        """
        Parameters
        ----------
        ...
        InteropUtils.invoke("streampropertiesconsumer_add_onchanged", self._pointer, value_func_wrapper_addr)
    ...
    _interop_func.restype = None
    _interop_func.argtypes = [c_void_p, c_void_p]
    def remove_OnChanged(self, value: Callable[[c_void_p, c_void_p], None]) -> None:
        """
        Parameters
        ----------
        ...
        if value is not None:
            ...
            value_func_wrapper_addr = ctypes.cast(value_converter_func_wrapper, c_void_p)
            if InteropUtils.DebugEnabled:
                print("Registering value_converter_func_wrapper in streampropertiesconsumer_remove_onchanged, addr {}".format(value_func_wrapper_addr))
                value_func_wrapper_addr_val = value_func_wrapper_addr.value
                # TODO StreamPropertiesConsumer._weakrefs.append(weakref.ref(value_converter_func_wrapper, lambda x: print("De-referenced value_converter_func_wrapper in streampropertiesconsumer_remove_onchanged, addr {}".format(value_func_wrapper_addr_val))))
        
        InteropUtils.invoke("streampropertiesconsumer_remove_onchanged", self._pointer, value_func_wrapper_addr)
        return value_func_wrapper_addr
