...
from .protocol_buffer import (Chunk, dtypes as _dtypes,
    ...
    for seq in data[:-1].split(b'\n\n'):
        ...
        if chunk.value is not None:
            ...
            years.extend(chunk.header.year
                         * np.ones(len(chunk.value[0]), dtype=np.int32))
...
def get_data(data, *, return_type='pandas', time_format='timestamp',
    ...
    args:
        return_type (str, optional) : requested data type (pandas|raw)
                                      Defaults to pandas
        time_format (str, optional) : requested time format
                                      (raw, timestamp, datetime).
    ...
    Example::
        ...
        with open('data.pb','rb') as f:
            header, values, seconds, nanos = get_data(f.read(),
                                                      return_type='raw')
    """
    ...
    if len(res) == 0:
        ...
        years = years[0] * np.ones(len(secs), dtype=np.int32)
...
class Archiver(ArchiverBasis):
    ...
    def _requestData(self, pvname, *, t0, t1, dtype='raw'):
        ...
        try:
            ...
            logger.error(
                'Failed to handle request {} reason {}'.format(request, e))
            raise e
