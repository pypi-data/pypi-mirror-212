...
class ArchiverInterface(metaclass=ABCMeta):
    ...
    def getData(self, var, * t0, t1, **kws):
        ...
        Args:
            ...
            padding (str, optional) :     restrict timestamp to requested
                                          time range (cuts first entry and
                                          adds dummy last entry)

            timezone (:class:`datetime.tzinfo`, optional):
                                          for pandas dataframes, use given
                                          tzinfo to translate the dataframe to.
                                          If none is given, translate it to
                                           the local timezone
