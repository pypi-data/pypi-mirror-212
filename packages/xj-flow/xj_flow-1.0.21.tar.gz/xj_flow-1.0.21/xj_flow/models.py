...
class FlowRule(models.Model):
    ...
    class Meta:
        db_table = 'flow_rule'
        verbose_name_plural = '9. 系统业务流程执行顺序配置'

    is_loop_flow_choices = [
        (1, '是'),
        (0, '否')
    ]
    id = models.AutoField(verbose_name='ID', primary_key=True)
    ...
    rebut_flows = models.JSONField(verbose_name='驳回流程', blank=True, null=True, default=[], help_text="驳回流程，驳回的流程将不会生效。必须重新执行。")
