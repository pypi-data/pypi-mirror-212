...
class BaseModel(Model):
    ...
    def __init__(self,{% for item in base_columns %}{% if loop.last %} {{item.COLUMN_NAME}}: {{item.DATA_TYPE}} = None{% else %} {{item.COLUMN_NAME}}: {{item.DATA_TYPE}} = None,{% endif %}{% endfor %}): {% for item in base_columns %}
...
'''
    ...
    def generate_with_tables(self, tables: Union[str, Iterable[str]], path: str = None, common_cols=['create_by', 'create_time'],
        ...
        base_columns = [v for v in attributes.values()] + common_cols
        base_columns = [col for col in base_columns if not (col in attributes and col in set(common_cols))]
    ...
    def _get_table_meta(self, table: str, common_cols):
        pk = None
        super_columns = []
