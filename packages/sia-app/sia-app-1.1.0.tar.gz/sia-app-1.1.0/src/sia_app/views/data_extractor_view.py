...
from sia_app.utils.global_constants import HOME_DATASETS_DIR, ASSETS_DIR
...
class DataExtractorView(ScrollableView):
  ...
  def load_view(self):
    ...
    label_text = 'Método de extracción:'
  ...
  def __setup_extraction(self):
    ...
    size_result = self.__get_adjusted_size()
    tk.messagebox.showinfo(
      title='Información de descarga',
  ...
  def __success_extract_callback(self, extract_details: ExtractionDetails):
    ...
    msg = \
      f'Descarga terminada.\nEstado de terminación: {"Descarga completa" if extract_details.complete else "Descarga parcial"}.\n'\
      f'Time min: {extract_details.time_min}.\nTime max: {extract_details.time_max}.\n'\
      f'Puede encontrar el dataset en: {fpath.resolve()}.'
    tk.messagebox.showinfo(
      title='Información de descarga',
      #message=f'Descarga terminada.\nVolúmen completo: {extract_details.complete}.\nTime min: {extract_details.time_min}\nTime max: {extract_details.time_max}\nPuede encontrar el dataset en: {fpath.resolve()}')
      message = msg)
  ...
  def __failure_extract_callback(self, err: BaseException):
    ...
    self.extract_button.configure(state='enabled')
    self.draw_map_button.configure(state='enabled')
    self.extractor.forget_tmp_files() # If any, just keep them in disk, but forget them in memory.
    self.__stop_progress_bar()
    tk.messagebox.showerror(
      title='Información de descarga',
