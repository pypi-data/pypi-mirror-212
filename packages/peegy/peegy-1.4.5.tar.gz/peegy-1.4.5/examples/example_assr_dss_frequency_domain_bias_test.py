...
from peegy.processing.pipe.general import ReferenceData, FilterData, AutoRemoveBadChannels, RemoveEvokedResponse
...
boot = pipeline['bootstrap'].output_node.statistical_tests['bootstrap']
print('\n Bootstrap HT2-Test \n'
      'n = {:} \n'
      'detections = {:} \n'
      'Detection rate {:} %'.format(boot.shape[0],
                                    np.sum(boot["p_value"] < 0.05),
                                    np.round(100 * np.sum(boot["p_value"] < 0.05) / boot.shape[0],
                                             decimals=2)))
