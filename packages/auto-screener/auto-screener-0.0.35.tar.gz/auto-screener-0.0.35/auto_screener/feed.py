...
from auto_screener.symbols import Separator
...
class MarketRecorder:
    ...
    def structure(self) -> Dict[str, List[str]]:
        ...
        return {
            exchange: list(tickers.keys())
            for exchange, tickers in self.market.items()
        }
    # end structure
    ...
    def data(self, exchange: str, symbol: str) -> pd.DataFrame:
        """
        Returns the market data of the ticker from the exchange.

        :param exchange: The source name of the exchange.
        :param symbol: The ticker of the pair.

        :return: The dataset of the spread data.
        """

        exchange = self.market[
            find_name(name=exchange, names=self.market.keys())
        ]

        return exchange[find_name(name=symbol, names=exchange.keys())]
    # end data
    ...
    def screener(
        ...
        if exchange not in self.market:
            raise ValueError(
                f"source {exchange} is not a valid exchange in {self}."
            )
        # end if

        symbol = find_name(name=symbol, names=self.market[exchange].keys())

        if symbol not in self.market[exchange]:
            raise ValueError(
                f"ticker {symbol} of exchange {exchange} "
        ...
        screener.market = self.market[exchange][symbol]
        ...
        for source in self.market:
            for ticker in self.market[source]:
                base_screeners.append(
                    self.screener(
                        symbol=ticker, exchange=source, delay=delay,
                        location=location, cancel=cancel
                    )
                )
            # end for
