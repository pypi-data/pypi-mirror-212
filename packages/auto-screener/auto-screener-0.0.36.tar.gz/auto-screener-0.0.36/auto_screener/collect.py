...
def _collect_exchange_assets(
    ...
    try:
        for ticker in feed.symbols():
            ...
            base, quote = symbol_to_parts(ticker, separator='-')
...
def _collect_exchange_symbols(
    ...
    for symbol in exchange_symbols:
        ...
        _, quote = symbol_to_parts(symbol)

        if (
            (quotes and (find_name(name=quote, names=quotes) in quotes)) or
            (excluded and (find_name(name=symbol, names=excluded) in excluded))
        ):
            continue
        # end if
