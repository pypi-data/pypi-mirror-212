...
def symbol_to_pair(symbol: str, separator: Optional[str] = Separator.value) -> Pair:
    ...
    :param symbol: The ticker to convert into a pair object.
    :param separator: The separator of the assets.
    ...
    if separator in symbol:
        ...
        raise ValueError(
            f"Cannot separate ticker '{symbol}' because "
...
def ticker_to_parts(symbol: str, separator: Optional[str] = Separator.value) -> Tuple[str, str]:
    """
    Converts a pair of assets into a ticker.
...
def reverse_ticker(symbol: str, separator: Optional[str] = Separator.value) -> str:
    """
    Converts a pair of assets into a ticker.
    ...
    base, quote = ticker_to_parts(symbol=symbol, separator=separator)
...
def adjust_symbol(symbol: str, separator: Optional[str] = Separator.value) -> str:
    ...
    :param symbol: The ticker of the asset to adjust.
    :param separator: The separator of the assets.
    ...
    if symbol.endswith(f'{separator}USD'):
        symbol = symbol.replace(f"{separator}USD", f"{separator}USDT")
    # end if
...
def parts_to_symbols(
    ...
    return [
        ...
        (parts_to_symbol_parts(bases, quotes, tickers))
    ]
