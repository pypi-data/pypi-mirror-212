...
class SigScanner:
    ...
    def tar_files(self):
        ...
        with tarfile.open(self.tar_output_path, 'w:gz') as tar:
            for f in tqdm.tqdm(self.files_to_tar, total=len(self.files_to_tar)):
                f_res = os.path.realpath(f)
                if f != f_res:
                    logging.debug("file and realpath are {} and {}".format(f,os.path.realpath(f)))
                tar.add(f_res)
    ...
    def build_sig_scanner_command(self):
        ...
        ## for versions of hub that are using signature scanner 8.x
        if self.version_grt:
    ...
    def upload_sig_json_results(self):
        ...
        try:
            if self.expand_sig_files:
                ...
                sig_path = os.stat(self.tar_output_path).st_size
                logging.debug("sig_path size is {}".format(sig_path))
    ...
    def upload_sig_bdio_results(self):
        ...
        try:
            if self.expand_sig_files:
                ...
                sig_path = os.stat(self.tar_output_path).st_size
                logging.debug("Sig_path size is {}".format(sig_path))
                logging.debug("sig files output path is {}".format(self.tar_output_path))
        except FileNotFoundError as e:
            util.error_and_exit(
                "FileNotFound error, please make sure yaml file is same in offline/online mode: {}".format(e))
