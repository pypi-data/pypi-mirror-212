...
class Permutation:
    ...
    """
    def __init__(self, x, y, n_perm=100, seed=1, n_jobs=1):
        ...
        if type(x) is not np.ndarray or type(y) is not np.ndarray:
            raise ValueError("Input is not a list or tuple, is {} instead".format(type(x)))
    ...
    def __call__(self, n_perm):
        ...
        return np.asarray([*surr_x]).squeeze(), np.asarray([*surr_y]).squeeze()
