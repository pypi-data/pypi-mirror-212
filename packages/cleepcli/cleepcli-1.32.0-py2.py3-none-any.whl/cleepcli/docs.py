...
# TODO /bin/cp -a "%(BUILD_DIR)s/xml/source/%(CORE)s.xml" "%(CORE)s-docs.xml"
    ...
    def publish_module_docs(self, module_name, module_version):
        """
        ...
        repo = self.__get_github_app_docs_repo()
        if repo is None:
    ...
    def __get_github_app_docs_repo(self):
        """
        ...
        try:
            ...
            github = Github(token)
            repo = github.get_repo('%s/%s' % (config.GITHUB_ORG, config.GITHUB_REPO_APP_DOCS))
