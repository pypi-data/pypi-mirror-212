...
class MaskSegmentation():
    ...
    def saptialbin_lasso_process(self):
        ...
        for i in gj['geometries']:
            ...
            tmp_outpath = self.outpath + f'\{label}'
            os.makedirs(os.path.join(tmp_outpath,
                        'segmentation'), exist_ok=True)
            self.gem_path = tmp_outpath + f'\segmentation'
            gef_outpath = tmp_outpath + f"\{self.sampleid}.{label}.label.gef"
            cg = CgefAdjust()
            ...
            if 'cellBin' in gef_f:
                ...
                for i in gj['geometries']:
                    ...
                    os.makedirs(os.path.join(
                        self.outpath, label), exist_ok=True)
                    tmp_outpath = os.path.join(
                        self.outpath, label) + f"/{self.sampleid}.lasso.cellbin.gef"
