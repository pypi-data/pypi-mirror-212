...
class AutocompleteCombobox(Combobox):
    ...
    def autocomplete(self, event, delta=0):
        ...
        #print(f"Pos {self.position}, get {self.get()}")
        if event.keysym.isdigit():
            self.user_data += event.keysym
        else:
            self.user_data += (self.get()[self.position])
        #if event.keysym == "??" or (platform.system() == "Linux" and event.keysym.startswith("U04") or event.keysym.isdigit()):
