...
def sendVideo(chat, result, use_url = False):
    if use_url:
        ...
        if not 100 < os.stat('tmp/video.mp4').st_size < 50 * 1024 * 1024:
            return []
...
def send_v2_imp(chat, result, rotate=0, send_all=False, no_cut=False, size_factor = None, rotate_detail = {}, start_page = 0, use_url = False):
    ...
    if result.video:
        try:
            ...
            if send_result:
                if not use_url:
                    print('album_sender video send download video helpful + 1', result.url)
                return send_result
            raise Exception('video send failed')
        except Exception as e:
            if use_url:
                raise Exception('try download video next time')
            print('album_sender video send download video not helpful + 1', result.url, e)
            raise e
