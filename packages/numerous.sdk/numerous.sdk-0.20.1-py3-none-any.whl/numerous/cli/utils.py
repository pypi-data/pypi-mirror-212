...
class OrganizationInterceptor(ClientInterceptor):
    def __init__(self, organization: Optional[str]):
    ...
    def intercept(
        ...
        if self._organization is not None:
            new_metadata = (call_details.metadata or []) + [
                ("numerous-organization", self._organization)
            ]
        else:
            new_metadata = call_details.metadata
...
def get_token_manager(
    api_url: str, organization_id: Optional[str]
) -> ContextManager[TokenManagerStub]:
    return StubClient(
        api_url,
        TokenManagerStub,
        extra_interceptors=[OrganizationInterceptor(organization_id)],
    )
