...
class DiveplaneCore:
    ...
    def __init__(  # noqa: C901
        ...
        **kwargs
        ...
        # If embedded, amalgam will be found within diveplane-local module
        if amlg_params.get('embedded'):
            ...
            embedded_amlg_location = Path(resource_filename(
                                          f'diveplane.local.lib.amlg.{operating_system}.{architecture}',
                                          library_filename))
            ...
            if embedded_amlg_location.exists():
                ...
                raise DiveplaneError(f'No amalgam library found in diveplane-local at {embedded_amlg_location}')
        ...
        # If version is set, but download not, use the default download location
            ...
            if versioned_amlg_location.exists():
                ...
                raise DiveplaneError(f'No amalgam library found in diveplane-local at {versioned_amlg_location}')
        ...
        # If embedded, core caml files will be found within diveplane-local module
        if core_params.get('embedded', False):
            if core_params.get('download'):
                _logger.warning('Incompatible params \'embedded\' and \'download\' set. '
                                'Disregarding download configuration')
            diveplane_embedded_filepath = Path(resource_filename('diveplane.local.lib.core',
                                                                 'diveplane.caml'))
            trainee_template_embedded_filepath = Path(resource_filename('diveplane.local.lib.core',
            ...
            if not (diveplane_embedded_filepath.exists()
                    and trainee_template_embedded_filepath.exists()):
                raise DiveplaneError('Core files diveplane.caml and trainee_template.caml '
