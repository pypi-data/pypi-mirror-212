...
******************************************************************************************
 ...
 2.0.00 - May-18-2023 petervh ddr-clips-1.1.15 'ddr-flags replaced iwth ddr-flags.yaml - not backward compatible with ddr-flags usecases'
 1.1.15 - May-3-2023 petervh 'Add get_notification method to ddrclass to enable a script to get the service impact notification after execution'
...
class DDR:
    def get_notification(self):
        '''
           Return a json representation of the "service impact notification" than contains
           FACTs collected and messages generated by the DDR usecase execution.
           
           The returned notification can be used by a Python application using FACTs and 
           conclusions produced by the DDR usecase

            :param none
        
        Usage::

            usecase_results = engine.get_notification() Where 'engine' is the DDR class instance

            :raises none:

        '''
        try:
            return_notify = self.si_notification
        except Exception as e:
            self.print_log("\n%%%% DDR Error: get_notification service-impact notification does not exist")

        return str(self.si_notification)
    ...
    #############################################################################
    #############################################################################
    def ddr(self,flist, single_run):
        self.ddr_init(flist, single_run)
    # 
    ...
    ######################################################################################
        while True:
            exit_ddr = self.engine_process()
    #
    # When the DDR main loop execution completes the engine_process method returns
    # Currently no additional actions are performed and a return is made to the 
    # code that ran the DDR main loop, e.g. ddrrun.py
    #
            if exit_ddr == 'exit': return
...
#
            ...
            for trigger in self.control["syslog-triggers"]:
                if self.control["debug-syslog"] == 1:
                    self.print_log("**** DDR Debug: Syslog trigger: " + str(trigger))
...
#
                if all([val in str(syslog) for val in trigger[0]]):
                    syslog_trigger = True
                    if self.control["debug-syslog"] == 1:
                        self.print_log("**** DDR Debug: Syslog trigger strings matched: " + str(trigger[0]))
                    try:
                        if trigger[2] == 'True':
                            if self.control["debug-syslog"] == 1:
                        ...
                        if trigger[1] != []:
                            try:
                                for syslog_fact in trigger[1]:
                                    if self.control["debug-syslog"] == 1:
                                        self.print_log("**** DDR Debug: Assert static Syslog FACT: " + str(syslog_fact))
                                    self.env.assert_string(str(syslog_fact))
                            except Exception as e:
                                self.print_log("%%%% DDR Error: Asserting static Syslog fact: " + str(trigger[1]) + "\n" + str(e))
                            break
                    except:
...
#  ['ring', 'str']
                            ...
                            try:
                                ...
                                fact1["timestamp"] = datetime.now().strftime("%m-%d-%Y_%H:%M:%S.%f")
                    ...
                    if self.control["debug-syslog"] == 1:
                        self.print_log("**** DDR Debug: Syslog did not match all triggering conditions :" + str(syslog) + "\n" + str(trigger[0]))
                    return
...
#######################################################################################
        if self.control["run-mode"] == 3:
            self.print_log("\n**** DDR Notice: Wait for trigger from external application control-file write to device")
...
#
                    try:
                        self.si_notification = self.control["service-impact"].format("DDR Service Impact Notification: " + str(self.control["use-case"]), impactMessage, self.clips_facts, self.dict_facts, str(self.control["session-time"]), datetime.now().strftime("%m-%d-%Y_%H_%M_%S.%f"))
                        if self.control["json-notify"] == 1:
                            data_dict = xmltodict.parse(self.si_notification)
                            self.si_notification = json.dumps(data_dict, indent=2, sort_keys=True)
                    except Exception as e:
                        self.print_log("\n%%%% DDR Error: Content error: " + str(e))
      
                    if self.control["show-notification"] == 1:
                        self.print_log("\n################################ DDR Service Impact Notification ############################\n")
                        self.print_log(self.si_notification)
#
...
#
                        try:
                            with open(filename, "a+") as fd:
                                fd.write(str(self.si_notification))
                                fd.write("\n")
                            if self.control["debug-notify"] == 1:
...
#
            if self.control["run-one"] == 1:
                self.print_log("\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!! DDR Single Execution Cycle Completed !!!!!!!!!!!!!!!!!!!!!!!!!!!\n")
                return 'exit' # Exit the DDR main loop
#
...
#
########################################################
    def ddr_init(self, flist, single_run):
    #
    ...
    #
        self.control = dict()
    #
    # if ddr-flags.yaml is in the local directory use the yaml file instead of ddr-flags
    #
        try:
            if os.path.exists('./ddr-flags.yaml'):
                with open('ddr-flags.yaml', 'r') as file:
                    self.control = yaml.safe_load(file)
                    self.control.update({"session-time" : str(datetime.now().strftime('%m-%d-%Y_%H_%M_%S.%f'))})
                    self.control.update({"session-log-count" : 0})
                    self.print_log("**** DDR Notice: ddr-flags.yaml in usecase directory used to set DDR control flags")
    #
    # if ddr-flags.yaml is not in the local directory use ddr-flags
    #
            else:
                self.print_log('\n%%%% DDR Error: DDR ddr-flags.yaml file does not exist')
                return

        except Exception as e:
            self.print_log('\n%%%% DDR Error: DDR ddr-flags.yaml file read error: ' + str(e))
            return # Exit the DDR main loop          
    ...
    #
            self.impactList = []
    #
    # Initialize the service-impact notfication response
    #
            self.control["service-impact"] = '''
  <ddr-notification xmlns="http://cisco.com/ns/yang/cisco-ddr-control">
...
{3}    </dict-facts>
    ...
    ###################################################################################################
        ...
        try:
            if os.path.exists('/bootflash/guest-share/ddr' + 'ddr-facts'):
                ...
                self.control.update({"flags-file" : '/bootflash/guest-share/ddr' + 'ddr-flags.yaml'})
                self.control.update({"devices-file" : '/bootflash/guest-share/ddr' + 'ddr-devices.yaml'})
            ...
            self.print_log('\n%%%% DDR Error: DDR control files read error: ' + str(e))
            return # Exit the DDR main loop          
    ...
    ###################################################################################################
        ...
        try:
    #
    # Use ddr-devices.yaml to initialize device definitions
    #
            if os.path.exists('./ddr-devices.yaml'):
                with open('ddr-devices.yaml', 'r') as file:
                    devices = yaml.safe_load(file)
                self.print_log("**** DDR Notice: ddr-devices.yaml in usecase directory used to set device information")
                device_list = []
                device_instance = []
                ...
                self.print_log('\n%%%% DDR Error: DDR ddr-devices.yaml file does not exists')
                return # Exit the DDR main loop

        except Exception as e:
            self.print_log('\n%%%% DDR Error: DDR ddr-devices.yaml file read error: ' + str(e))
            return # Exit the DDR main loop
    ...
    #
            ...
            try:
                self.control.update({"parse-regex-list" : inputdata.parse_regex_list})
            except: pass # Optional ddr-facts content
    ...
    #
        try:
            ...
            self.print_log("**** DDR Notice: Inference Engine rules file loaded: " + self.control["rules-file"])
        except Exception as e:
        ...
        for each in filtered_instances:
            ...
            for slot in slots:
                ...
                if protofact["types"][slot] == "int":
                    ...
                    protofact["slots"][slot] = str(value)
...
#############################################################################
    ...
    #
        try:
            ...
            version = self.control["device-list"][int(device_index)][10]
    ...
    #
                    try:
                        if self.control["debug-parser"] == 1:
                            self.print_log("**** DDR Debug: show_and_assert_fact raw data: \n\n" + str(response) + "\n")
                        parsed_genie_output = parser.parse(output=response, pversion=version, debug=self.control["debug-parser"])
                    except Exception as e:
                    ...
                    if parsed_genie_output == {}:
                        self.print_log("\n%%%% DDR Error: show_and_assert_fact no parser output: " + str(fact["genie_parser"]))
                        return
...
#############################################################################
    ...
    #
    #############################################################################
    def assert_syslog_fact(self, syslog_message):
        try:
            version = str(self.control["mgmt-device"][8]) # use the OS version in the mgmt-device definition
            if self.control["debug-syslog"] == 1:
                self.print_log("**** DDR Debug: assert_syslog_fact device-type and version: " + str(self.control["mgmt-device"][7]) + " " + str(version))
                self.print_log("**** DDR Debug: assert_syslog_fact syslog_message: " + str(syslog_message))

            if self.control["mgmt-device"][7] == 'XR':
                try:
                    parser = genie_str_to_class("ParseXRSyslogMessage")
                    parsed_genie_output = parser.parse(output=syslog_message, pversion=version, debug=self.control["debug-parser"])
                    parsed_genie_output["device"] = self.control["mgmt-device"][4]
                    parsed_genie_output["timestamp"] = datetime.now().strftime("%m-%d-%Y_%H:%M:%S.%f")
                    if parser == None:
                        self.print_log("\n%%%% DDR Error: Exception in assert_syslog_fact: parser not found: ParseXRSyslogMessage")
                        return
                except Exception as e:
                    self.print_log("\n%%%% DDR Error: Exception in assert_syslog_fact: ParseXRSyslogMessage parsing error: " + str(syslog_message) + "\n" + str(e))
                    return
                if self.control["debug-syslog"] == 1:
                    self.print_log("**** DDR Debug: assert_syslog_fact result: \n" + str(parsed_genie_output))                  
    # Assert the Syslog FACT
                try:
                    template = self.env.find_template("syslog-message") # get an empty template fact for event
                    template.assert_fact(**parsed_genie_output)
                except Exception as e:
                    self.print_log("\n%%%% DDR ERROR: assert_syslog_fact error: " + str(e) + "\n")
                return
                ...
                try:
                    ...
                    parsed_genie_output["timestamp"] = datetime.now().strftime("%m-%d-%Y_%H:%M:%S.%f")
                    if parser == None:
                        self.print_log("\n%%%% DDR Error: Exception in assert_syslog_fact: parser not found: ParseXESyslogMessage")
                        return
                except Exception as e:
                    self.print_log("\n%%%% DDR Error: Exception in assert_syslog_fact: ParseXESyslogMessage parsing error: " + str(syslog_message) + "\n" + str(e))
                    return
                if self.control["debug-syslog"] == 1:
                    self.print_log("**** DDR Debug: assert_syslog_fact result: \n" + str(parsed_genie_output))                  
    # Assert the Syslog FACT
            ...
            elif self.control["mgmt-device"][7] == 'NX':
                try:
                    parser = genie_str_to_class("ParseNXSyslogMessage")
                    parsed_genie_output = parser.parse(output=syslog_message, pversion=version, debug=self.control["debug-parser"])
                    parsed_genie_output["device"] = self.control["mgmt-device"][4]
                    parsed_genie_output["timestamp"] = datetime.now().strftime("%m-%d-%Y_%H:%M:%S.%f")
                    if parser == None:
                        self.print_log("\n%%%% DDR Error: Exception in assert_syslog_fact: parser not found: ParseNXSyslogMessage")
                        return
                except Exception as e:
                    self.print_log("\n%%%% DDR Error: Exception in assert_syslog_fact: ParseNXSyslogMessage parsing error: " + str(syslog_message) + "\n" + str(e))
                    return
                if self.control["debug-syslog"] == 1:
                    self.print_log("**** DDR Debug: assert_syslog_fact result: \n" + str(parsed_genie_output))                  
    # Assert the Syslog FACT
    ...
    def assert_statistics_fact(self, template_name, statistics):
        try:
            ...
            try:
                fact["timestamp"] = datetime.now().strftime("%m-%d-%Y_%H:%M:%S.%f")
    ...
    def print_log(self, logMessage):
        try: 
            ...
            if self.control["debug-logging"] >= 1: 
                log_path = self.control["log-path"] + self.control["log-file"] + "_TS_" + self.control["session-time"]
                with open(log_path, 'a+') as debuglog_file:
...
#
    ...
    def run_decode_btrace_log(self, fact_index, device_index, show_template, pcount, par1, par2, par3, device_id):
        ...
        if self.control["actions"] == 1:
            ...
            access_type = str(self.control["device-list"][int(device_index)][5])
            version = str(self.control["device-list"][int(device_index)][10])
    ...
    #
                        try:
                            if self.control["debug-parser"] == 1:
                                self.print_log("**** DDR Debug: run_decode_btrace_log raw data: \n\n" + str(response) + "\n")
                            parsed_genie_output = parser.parse(output=response, pversion=version, debug=self.control["debug-parser"])
                        ...
                        if parsed_genie_output == {}:
                            self.print_log("\n%%%% DDR Error: run_decode_btrace_log no parser output: " + str(fact["genie_parser"]))
                            return
        #
    ...
    #
                    try:
                        if self.control["debug-parser"] == 1:
                            self.print_log("**** DDR Debug: run_logging_trigger raw data: \n\n" + str(response) + "\n")
                        parsed_genie_output = parser.parse(num_lines=int(number_lines), pversion=version, output=response, facility_filter=str(facility))
                    except Exception as e:
   ...
   #
    ...
    #
            try:
                ...
                try:
                    if self.control["debug-parser"] == 1:
                        self.print_log("**** DDR Debug: run_process_file raw data: \n\n" + str(file_data) + "\n")
                    parsed_genie_output = parser.parse(output=file_data, pversion=version, debug=self.control["debug-parser"])
                except Exception as e:
    ...
    def run_read_control_file(self, control_file, delay):
        ...
        try:
            while not os.path.exists(str(control_file)):
                if self.control["debug-control"] == 1:
    ...
    def run_show_parameter(self, fact_index, device_index, show_template, pcount, par1, par2, par3, no_fact, yes_fact):
        ...
        Usage::
              ...
              (run_show_parameter 3 0 "show monitor capture CAP buffer brief" 0 none none none "(fact-found (value false))" "(fact-found (value true))")
        ...
        #
                    if str(access_type) == 'cli':
                        ...
                        if self.control["debug-CLI"] == 1:
                            self.print_log("**** DDR Notice: CLI Command Executed: " + str(command) + " at: " + str(timestamp))
                            self.print_log("**** DDR Notice: run_show_paramter CLI Command Response: \n" + str(response))
                    else:
        #
        # Use SSH to run the show command
        #
                        try:
                            ...
                            response = child.before
                            if self.control["debug-CLI"] == 1:
                                self.print_log("**** DDR Notice: run_show_parameter CLI Command Response:\n " + str(response))
    ...
    #
                        try:
                            if self.control["debug-parser"] == 1:
                                self.print_log("**** DDR Debug: run_show_parameter raw data: \n\n" + str(response) + "\n")
                            parsed_genie_output = parser.parse(output=response, pversion=version, debug=self.control["debug-parser"])
                        except Exception as e:
                        ...
                        if parsed_genie_output == {}:
                            self.print_log("\n%%%% DDR Error: run_show_parameter: no parser output: " + str(fact["genie_parser"]))
                            return
        #
        # Get the starting point in the Python dictionary containing the parsed content
        #
    ...
    def run_show_parameter_index(self, fact_index, device_index, show_template, pcount, par1, par2, par3, no_fact, yes_fact, command_timeout):
        """
            Rule file function call: (run_show_parameter_index 3 1 "show platform software fed active vp key {0} {1}" 2 ?key ?vlan none no-fact no-fact, 120)
            ...
            :param no_fact: Optional fact to assert if this function did not assert a fact that met rule conditions
            :param yes_fact: Optional fact to assert if this function did assert a fact that met rule conditions
            :param command_timeout: 0 to use default timeout of 60 seconds otherwise number of seconds before command timeout
        ...
        #
            ...
            if int(command_timeout) == 0:
                ...
                show_timeout = int(command_timeout)
        ...
        #
                    if device_access == 'cli':
                        if device_index == 0:
                            response = cli.cli(command, "30")
                            if self.control["debug-CLI"] == 1:
                                self.print_log("**** DDR Notice: run_show_paramter_index CLI Command Executed: " + str(command) + " at: " + str(timestamp))
                                self.print_log("**** DDR Notice: run_show_paramter_index CLI Command Response: \n" + str(response))
                        else:
        ...
        # Note: 60 seconds is default timeout for show command execution
        #
                        try:
                            options = '-q -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null -oPubkeyAuthentication=no'
                            ...
                            child = pexpect.spawn(ssh_cmd, timeout= show_timeout, encoding='utf-8')
                            child.delaybeforesend = None
                            if self.control["debug-CLI"] == 1:
                                child.logfile = sys.stdout
                            child.expect(['\r\nPassword: ', '\r\npassword: ', 'Password: ', 'password: '])
    ...
    #
                        try:
                            if self.control["debug-parser"] == 1:
                                self.print_log("**** DDR Debug: run_show_parameter_index raw data: \n\n" + str(response) + "\n")
                            parsed_genie_output = parser.parse(output=response, debug=self.control["debug-parser"])

                        except Exception as e:
                            self.print_log("\n%%%% DDR Error: Exception in run_show_parameter_index for parser: " + str(fact["genie_parser"]) + " " + str(e))
                            return
                        if self.control["debug-parser"] == 1:
                        ...
                        if parsed_genie_output == {}:
                            self.print_log("\n%%%% DDR Error: run_show_parameter_index no parser output: " + str(fact["genie_parser"]))
                            return
        #
        # Get the starting point in the Python dictionary containing the parsed content
        #
    ...
    def run_show_parameter_index_version(self, fact_index, device_index, show_template, pcount, par1, par2, par3, no_fact, yes_fact, command_timeout, version):
        """
            Rule file function call: (run_show_parameter_index 3 1 "show platform software fed active vp key {0} {1}" 2 ?key ?vlan none no-fact no-fact, 120)
        ...
        if self.control["actions"] == 1:
            if self.control["debug-CLI"] == 1:
                self.print_log("**** DDR Debug: run_show_parameter_index_version: show_template: " + str(show_template))
            index = int(fact_index)
            fact = self.control["show-parameter-fact-list"][int(index)]
            timestamp =  datetime.now().strftime("%m-%d-%Y_%H:%M:%S.%f")
        #
        ...
        #######################################################################
            try:
                cmdline = str(show_template)
                if self.control["debug-CLI"] == 1:
                    self.print_log("**** DDR Debug: run_show_parameter_index_version: cmdline: " + str(cmdline))
        #
        # substitute parameters in command string if included in call from rule
        #
                if int(pcount) == 0:
                ...
                try:
                    if self.control["debug-CLI"] == 1:
                        self.print_log("**** DDR Debug: run_show_parameter_index_version: command: " + str(command))
        #
        # If access-type is cli use the Python CLI package to run command
        # The device_index must be 0 to run on the local device
        #
                    if device_access == 'cli':
                        if device_index == 0:
                            response = cli.cli(command, "30")
                            if self.control["debug-CLI"] == 1:
                                self.print_log("**** DDR Notice: run_show_paramter_index_version CLI Command Executed: " + str(command) + " at: " + str(timestamp))
                                self.print_log("**** DDR Notice: run_show_paramter_index_sersion CLI Command Response: \n" + str(response))
                        else:
                            self.print_log("\n%%%% DDR ERROR: run_show_parameter_index_version: device_index not 0 for local access: " + str(device_index))
                            return                         
                    else:
        #
        # Use SSH to run the show command
        ...
        #
                        try:
                            ...
                            child.close()
                            self.print_log("\n%%%% DDR ERROR: run_show_parameter_index_version SSH or timeout Error: " + str(ssh_cmd) + "\n")
                            return
         #
         # Assert the FACTs for using the response
         #
                    ...
                    try:
                        if "error" in response:
                            self.print_log("\n%%%% DDR Error: error in run_show_parameter_index_version show command response: " +str(device_id) + " " + str(command) + " response: " + str(response))
                            return
                        parser = genie_str_to_class(fact["genie_parser"])
                        if parser == None:
                            self.print_log("\n%%%% DDR Error: run_show_parameter_index_version: parser not found: \n" + str(fact["genie_parser"]) + " \n")
                            return
    #
    # Parse data if the parser is available
    #
                        try:
                            if self.control["debug-parser"] == 1:
                                self.print_log("**** DDR Debug: run_show_parameter_index_version raw data: \n\n" + str(response) + "\n")
                            parsed_genie_output = parser.parse(output=response, pversion=version, debug=self.control["debug-parser"])
                        except Exception as e:
                            self.print_log("\n%%%% DDR Error: Exception in run_show_parameter_index_version parser not found: " + str(fact["genie_parser"]))
                            return
                        if self.control["debug-parser"] == 1:
                            self.print_log("**** DDR Debug: run_show_parameter_index_version parsing result: \n\n" + str(parsed_genie_output) + "\n")

                        if self.control["debug-CLI"] == 1:
                            self.print_log("**** DDR Debug: run_show_parameter_index_version genie: " + str(parsed_genie_output) + "\n")
                        if parsed_genie_output == {}:
                            if self.control["debug-parser"] == 1:
                                self.print_log("\n**** DDR Notice: run_show_parameter_index_version no parser output: " + str(fact["genie_parser"]))
                            return
        #
        # Get the starting point in the Python dictionary containing the parsed content
        #
	...
	#
                                    try:
                                        ...
                                        self.print_log("\n%%%% DDR Error: Exception in run_show_parameter_index_version slot replace: " + str(protofact["slots"][slot]) + " " + str(e))

                                fact_found = True
                                self.assert_template_fact(protofact, 'device', str(self.control["device-list"][int(device_index)][4]), sub_dictionary)

                    except Exception as e:
                        self.print_log("\n%%%% DDR Error: Exception in run_show_parameter_index_version response processing: " + str(e) + " slot: " + str(slot))
                        return

                except Exception as e:
                    self.print_log("\n%%%% DDR ERROR: run_show_parameter_index_version Error: Exception sending show command: " + str(e) + "\n")
            except Exception as e:
                self.print_log("\n%%%% DDR ERROR: run_show_parameter_index_version Error: Exception generating show command: " + str(e) + "\n")    
    ...
    def run_suffix(self, fact_string, fact_prefix):
        ...
        try:
            ...
            if len(result) == 2:
             ...
             self.print_log("\n%%%% DDR ERROR: run_prefix string/prefix: " + str(fact_string) + " " + (fact_prefix) + "\n" + str(e))
    ...
    def run_split_string(self, text_string, split_char):
        """
            Rule function call: (run_split_string text_string split_char)
            
            Invoked in the "RHS" of a triggered rule.
            This function applies splits a string at the first occurence of the split_char and returns the first instance or entire string if no split_char
                                    
            :text_string: Text string to process
            :split_char: character to split string
        
        Usage::

              (run_split_string "9.3(6)_[build_9.3(5.92)]", "_")
        ...
        try:
            strings = text_string.split(split_char)
            return str(strings[0])
        except Exception as e:
            self.print_log("\n%%%% DDR ERROR: run_regex: " + str(text_string) + "\n")

    def run_get_timestamp(self):
        """
            Rule function call: (run_get_timestamp)
            
            Invoked in the "RHS" of a triggered rule.
            This function returns a timestamp string to insert into the timestamp slot in a FACT
        ...
        try:
            ...
            self.print_log("\n%%%% DDR ERROR: run_get_timestamp: " + str(e) + "\n")

    def run_parse(self, string_input, expression):
        """
            Rule function call: (run_parse string_to_parse expression)
            
            Invoked in the "RHS" of a triggered rule.
            This function returns a single value if the value is present in the string_to_parse, 'not_found' if not present

        Parameters::
            :string_to_parse: String expected to contain a value that matches the regular expression
            :expression: Index into parse_regex_list in ddr-facts for regex expression to extract value into the variable 'parsed_value
            
        Usage::

              (run_parse ?string_to_parse ?regex)
              (run_parse "Received IMET notification for peerIP: 204.1.1.1, BD: 1002" 1)
        ...
        try:
            regex = self.control["parse-regex-list"][int(expression)]
            parsed_value = "not_found"
            try:
                p1 = re.compile(regex)
            except Exception as e:
                self.print_log("\n%%%% DDR Error: run_parse expression compile: " + str(e) + "\nexpression: " + str(expression))
                return parsed_value            
            m = p1.match(string_input.rstrip())

            if m:
                group = m.groupdict()
                parsed_value = str(m.groupdict()['parsed_value'])

        except Exception as e:
            self.print_log("\n%%%% DDR Error: run_parse: " + str(e) + "\nmessage: " + str(string_input) + "\nexpression: " + str(expression))
        return parsed_value
