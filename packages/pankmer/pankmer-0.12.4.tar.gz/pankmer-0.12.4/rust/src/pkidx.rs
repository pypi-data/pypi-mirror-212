...
pub fn run_index(genomes_input: &str, genomes: PKGenomes, outdir: &str, fraction: f64,
    ...
    for core_blocks in all_core_blocks.chunks(disk_segment_size) {
        ...
        let post_dict = index_genomes(core_blocks, &genomes, kmersize,
            fraction, &outdir, threads, index, &tar_file).expect("couldn't index genomes");
        print_err("Finished Indexing (segment).");
        print_err("Concatenating files (segment).");
        let pd = concat_files(post_dict, core_blocks, &outdir)?;
...
fn index_genomes(all_core_blocks: &[Vec<u64>], genomes: &PKGenomes, kmersize: usize, kmer_fraction: f64,
                 outdir: &str, threads: usize, index: &str,
...
fn concat_files(post_dict: HashMap<String, PKTbl>, all_core_blocks: &[Vec<u64>], outdir: &str) -> PyResult<PKTbl> {
    ...
    let mut kmers_out = match fs::OpenOptions::new().append(true).open(&kmers_out_path) {
        Ok(buf) => niffler::get_writer(Box::new(buf), niffler::compression::Format::Gzip, niffler::Level::Nine).expect("Can't open file for writing"),
        Err(_) => niffler::to_path(&kmers_out_path, niffler::compression::Format::Gzip, niffler::Level::Nine).expect("Can't open file for writing")
    };
    let mut indices_out = match fs::OpenOptions::new().append(true).open(&indices_out_path) {
        Ok(buf) => niffler::get_writer(Box::new(buf), niffler::compression::Format::Gzip, niffler::Level::Nine).expect("Can't open file for writing"),
        Err(_) => niffler::to_path(&indices_out_path, niffler::compression::Format::Gzip, niffler::Level::Nine).expect("Can't open file for writing")
    };
