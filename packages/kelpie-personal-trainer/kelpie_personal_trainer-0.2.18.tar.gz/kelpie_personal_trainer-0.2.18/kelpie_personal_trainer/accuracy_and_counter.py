...
def check_situp_distance(result):
    right_shoulder = 100
    right_knee = 100
    left_shoulder = 100
    left_knee = 100
...
def count_situp(result):
    ...
    try:
        ...
        distance = check_situp_distance(result)
...
SQUAT_FOOT_LOWER_THRESHOLD = 0
SQUAT_FOOT_UPPER_THRESHOLD = 110
SQUAT_DISTANCE_THRESHOLD = 0.333
...
def squat(result, img, last_status, counter):
    ...
    if len(xd_2) == 0:
        xd_2.append(result_obj['status'])
    else:
        if xd_2[len(xd_2) - 1] and not (result_obj['status']):
            xd_2.append(result_obj['status'])
        if not xd_2[len(xd_2) - 1] and (result_obj['status']):
            xd_2.append(result_obj['status'])
