...
FOOT_LOWER_THRESHOLD = 40
FOOT_UPPER_THRESHOLD = 150
BODY_LOWER_THRESHOLD = 40
BODY_UPPER_THRESHOLD = 150
SITUP_DISTANCE_THRESHOLD = 1
...
def sit_up(result, img, last_status, counter):
  ...
  if (status is (not last_status['situp'])) and status and (not is_down):
      counter['situp'] += 1

  last_status['situp'] = status
...
def extract_features(video_clips):
    ...
    for clip in video_clips:
        ...
        features.append(base_model.predict(preprocessed_clip))
...
def count_accuracy_pushup(video, max_frames=30, resize=(img_size, img_size)):
    ...
    predicted = pushup_model.predict(np.expand_dims(extracted_features, axis=1))
...
def count_accuracy_situp(video, max_frames=30, resize=(img_size, img_size)):
    ...
    predicted = situp_model.predict(np.expand_dims(extracted_features, axis=1))
...
def count_accuracy_squat(video, max_frames=30, resize=(img_size, img_size)):
    ...
    predicted = squat_model.predict(np.expand_dims(extracted_features, axis=1))
...
def check_move(move):
    ...
    cap = cv2.VideoCapture('./test-squat.mp4')
    move_arr = []
    ...
    while True:
        ...
        if len(move_arr) == 30:
            if detected_move['pushup']:
                accuracy['pushup'].append(count_accuracy_pushup(move_arr))
            if detected_move['situp']:
                accuracy['situp'].append(count_accuracy_situp(move_arr))
            if detected_move['squat']:
                accuracy['squat'].append(count_accuracy_squat(move_arr))
