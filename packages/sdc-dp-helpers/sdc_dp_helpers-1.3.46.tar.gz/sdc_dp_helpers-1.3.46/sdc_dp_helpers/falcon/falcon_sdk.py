# pylint: disable=too-few-public-methods,arguments-differ,import-error,too-many-arguments,too-many-locals,line-too-long
""" Falcon Reader SDK"""
...
class ChannelIdsV1(RequestHandler):
    ...
    def make_api_call(self, **kwargs):
        ...
        if response.status_code == 429 and 'Retry-After' in response.headers:  # Check if we hit the rate limit
            os.environ['API_WAIT_TIME'] = str(
                response.headers.get('Retry-After', 1))
            raise FalconTimeoutException("Rolling Window Quota [429] reached.")
            ...
            if response.status_code != 200:
                if response.status_code == 429 and 'Retry-After' in response.headers:  # Check if we hit the rate limit
                    ...
                    raise FalconTimeoutException(
                        "Rolling Window Quota [429] reached.")
                raise ConnectionError(
                    f"Falcon API failed to return channel ids. "
                    f"Status code: {response.status_code}, Reason: {response.reason}."
                )
