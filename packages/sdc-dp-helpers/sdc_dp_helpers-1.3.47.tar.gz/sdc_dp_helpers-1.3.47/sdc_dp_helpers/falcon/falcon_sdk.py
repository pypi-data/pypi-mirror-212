# pylint: disable=too-few-public-methods,arguments-differ,import-error,too-many-arguments,too-many-locals,line-too-long, disable=useless-elif-before-else, disable=R1705
""" Falcon Reader SDK"""
...
class ChannelIdsV1(RequestHandler):
    ...
    def make_api_call(self, **kwargs):
        ...
        if int(response.status_code) == 429 and 'Retry-After' in response.headers or 'Too Many Requests' in response.reason:
            wait_time = response.headers.get(
                'Retry-After', randint(4, 29)) * (1 + randint(2, 8) * random())
            os.environ['API_WAIT_TIME'] = str(wait_time)
            raise FalconTimeoutException(FalconTimeoutException(
...
class ContentIdByChannelId(RequestHandler):
    ...
    def make_api_call(
        ...
        while endpoint_url is not None:
            ...
            elif int(response.status_code) == 429 and 'Retry-After' in response.headers or 'Too Many Requests' in response.reason:
                wait_time = response.headers.get(
                    'Retry-After', randint(4, 29)) * (1 + randint(2, 8) * random())
                os.environ['API_WAIT_TIME'] = str(wait_time)
                raise FalconTimeoutException(FalconTimeoutException(
    ...
    def make_api_call(
        ...
        while endpoint_url:
            ...
            if int(response.status_code) == 200:
                ...
                break
            elif status_code == 429 and 'Retry-After' in response.headers or 'Too Many Requests' in response.reason:
                wait_time = response.headers.get(
                    'Retry-After', randint(4, 29)) * (1 + randint(2, 8) * random())
                os.environ['API_WAIT_TIME'] = str(wait_time)
                raise FalconTimeoutException(FalconTimeoutException(
