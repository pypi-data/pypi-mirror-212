...
def str_dict(dicts, str1='', pre=''):
    ...
    if type(dicts) == dict:
        ...
        pre = f'{len(dicts)} item in the {type(dicts)}:' if pre is None else f'{pre}'
        items = [pre]
        for key in dicts.keys():
            items.append(f'{key}:{str_dict(dicts[key], str1 + "  ", pre=pre)},')
    ...
    # 判断是否可以迭代
        ...
        items = [pre]
        for item in dicts:
            items.append(f'{str_dict(item, str1 + "  ", pre=pre)},')        
        str_oneLine = ''.join(items)
        if len(str_oneLine) <= 120:
            return f'{{{str_oneLine}}}'
        str_item = f'\n  {str1}'.join(items)
