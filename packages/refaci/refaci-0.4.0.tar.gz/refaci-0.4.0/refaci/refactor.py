...
def refactor(
    ...
    replacements: dict["FilePathRegex | ContentsRegex", Sequence[tuple[str, str]]] = {},
):
    dir_path_replacements = {
        regex: replacements for regex, replacements in replacements.items() if isinstance(regex, DirPathRegex)
    }
    file_path_replacements = {
        regex: replacements for regex, replacements in replacements.items() if isinstance(regex, FilePathRegex)
    }
    contents_re = {
