...
def wrap_dispatch_request(origin):
    ...
    def handler(self):
        ...
        if err is not None:
            try:
                active_object = jennifer_agent().current_active_object()

                if active_object is not None:
                    profiler = active_object.profiler

                    if type(err) == NotFound:
                        active_object.http_status_code = 404
                        ...
                        active_object.profiler.add_service_error_profile("Service Error: " + ex_result)
            except:
                pass
