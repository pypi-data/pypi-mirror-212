...
def collect_dataset(working_dir, input_files, dictionary, output_path, mode='collect_dataset'):
    ...
    with open(os.path.join(working_dir, 'training_words.tsv'), 'w', newline='') as tw:
...
def get_words(working_dir, out_path):
    subprocess.call(["Rscript", os.path.join(working_dir, "scripts/get_words.R"), working_dir + '/', out_path + '/'])
