...
class SnapProcessor:
    ...
    def submit_workflow(
        self, parameters: SnapParameters, user_id: str, job_id: str
    ) -> str:
        ...
        manifest = IoArgoprojWorkflowV1alpha1Workflow(
            metadata=ObjectMeta(generate_name="snap-"),
            spec=IoArgoprojWorkflowV1alpha1WorkflowSpec(
                workflow_template_ref=IoArgoprojWorkflowV1alpha1WorkflowTemplateRef(
                    # see name in
                    # faas-snap/deploy/templates/WorkflowTemplates/snap.yaml
                    name="faas-snap"
                ),
                arguments=IoArgoprojWorkflowV1alpha1Arguments(
                    parameters=[
                        IoArgoprojWorkflowV1alpha1Parameter(
                            name="snap_parameters", value=parameters.json()
                        ),
                        IoArgoprojWorkflowV1alpha1Parameter(
        ...
        logger.info(f"Submitted snap workflow with ID {workflow_name}")
