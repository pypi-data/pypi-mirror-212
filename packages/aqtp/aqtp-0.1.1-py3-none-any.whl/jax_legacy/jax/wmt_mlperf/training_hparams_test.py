...
class HParamsTest(parameterized.TestCase):
  ...
  # BEGIN GOOGLE-INTERNAL
  def test_load_all_configs_in_hparams_configs_dir(self):
    loaded_at_least_one_config = False
    for root, _, filenames in resources.WalkResources(train.CONFIG_DIR):
      loaded_at_least_one_config = True
      root = gpath.GPath(root)
      for filename in filenames:
        filename = gpath.GPath(filename)
        if filename.suffix == '.json':
          full_file_path = root / filename
          hparams_blob = resources.GetResource(full_file_path, mode='rt')
          training_hparams_inst = hparams_utils.load_dataclass_from_disk(
              training_hparams.TrainingHParams, full_file_path)
          self.assertJsonEqual(
              hparams_blob,
              json.dumps(dataclasses.asdict(training_hparams_inst)))
    self.assertTrue(
        loaded_at_least_one_config,
        'No hparams configs were found. train.CONFIG_DIR or data build rules for '
        'the tests are misconfigured.')
  ...
  def test_convert_lists_to_tuples(self):
    ...
    test_output = hparams_utils._convert_lists_to_tuples(test_input)
    self.assertEqual(
        test_output,
        {
            'a': (
