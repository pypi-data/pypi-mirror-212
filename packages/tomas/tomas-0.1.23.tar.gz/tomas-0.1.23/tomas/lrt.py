...
def total_mRNA_aware_DE(adata_rc, groupby, reference=None, groups=None, minCells=5,n_cores=None,pval_cutoff=0.05, logFC_cutoff=0,method='wilcoxon'):
    ...
    if method=='wilcoxon':
        ...
        df = pd.DataFrame(pct_each_type,index=adata_rc.var_names,columns=[g+'_pct' for g in groups])
        ...
        for idx in range(1,len(groups)):
            ...
            df[group+'_pval_adj'] = np.nan
            df.loc[adata_rc.var_names[goverMcells],group+'_pval_adj'] = sim_lrt_pval_adj            
...
def k_marg_pmf(k_arr, bb_para, logN_para, c, g):
    ...
    k_pmf_r = [quad(integrand, mu-3*sigma,mu+3*sigma,args=(k,alpha,beta,mu,sigma),epsabs=1e-6,epsrel=1e-6,limit=1000)[0] for k in range(k_max)]
...
def summarize2DE(gs_df, lrt_df, group, pval_cutoff = 0.05, logFC_cutoff = 0):
    ...
    de_df = pd.DataFrame({'log2FC_gs':gs_df[group+'_logfoldchanges'],
                          ...
                          'pval_adj_rc':lrt_df[group+'_pval_adj'],
