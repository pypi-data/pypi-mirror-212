...
class DecoderParamsBuilder():
    ...
    def set_knowledge(self, knowledge_dict: Dict, is_json=False, life_cycle=0, life_cycle_turns=0):
        if self._output_proto_type == RemoteDecoderOutput:
            knowledge = ProtoWrapper.wrap_knowledge(knowledge_dict, is_json, life_cycle, life_cycle_turns)
            self._output_proto.knowledge.CopyFrom(knowledge)
        else:
            ModelContextParser.set_knowledge(knowledge_dict, is_json, life_cycle, life_cycle_turns, self._output_proto)
