...
class Inference:
    def __init__(self, cfg_string, weight_file, 
                 GS_ACCESS_KEY_ID=None, GS_SECRET_ACCESS_KEY=None, logger=None):
    ...
    def segmentation(self, tif_path):
        ...
        if self.logger:
            self.logger.info(f"Total number of patches for image {tif_path} are: {len(patches)}")
        ...
        if self.logger:
            self.logger.info(f"Full mask's dimensions are {full_mask.shape}")
