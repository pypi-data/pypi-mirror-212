...
class MsbQueue:
	...
	def publish(self, task_name, data, **kwargs):
		self.celery.send_task(name=task_name, queue=self.queue, args=[data], **kwargs)
...
class QueueService(MsbService):
	...
	def broadcast_to(self, task_and_queue: dict, data, **kwargs):
		for task, queue in task_and_queue.items():
			MsbQueue(queue=queue).publish(task_name=task, data=data, **kwargs)
