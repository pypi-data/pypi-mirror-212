...
def serialize_dict(obj, indent, new_indent):
    if len(obj) == 0:
        ...
        for key in list(obj)[:len(obj)-1]:
            result += ' ' * new_indent + '<' + serialize(key, indent, new_indent) + '>' +  \
                      serialize(obj[key], indent, new_indent) + '</' + serialize(key, indent, new_indent) + '>' + '\n'
        result += ' ' * new_indent + '<' + serialize(list(obj)[len(obj)-1], indent, new_indent) + '>' + \
                  serialize(obj[list(obj)[len(obj)-1]], indent, new_indent) + '</' + \
                  serialize(list(obj)[len(obj)-1], indent, new_indent) + '>' + '\n'
