...
class FTPClient:
    ...
    async def init_connection(self, host, port, credentials: Dict, ssl: Callable = None):
        """
        ...
        try:
            ...
            raise ComponentError(
                f'FTP connection failed: {err!s}'
            ) from err
        except Exception as err:
            raise ComponentError(
                f'FTP Exception: {err!s}'
            ) from err
        try:
    ...
    def err_handler(self, err):
        print(
            f'FTP Error: reason: {err.reason}, error: {err}'
        )
        return False
    ...
    async def download_file(self, file: str, destination: str, rewrite: bool = False):
        ...
        try:
            ...
            raise ComponentError(
                f'FTP Download Exception: {err!s}'
            ) from err
