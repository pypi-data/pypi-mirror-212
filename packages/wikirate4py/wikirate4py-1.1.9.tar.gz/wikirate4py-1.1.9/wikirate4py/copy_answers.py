...
def conflict_free(verified_answers_ids, entity):
    ...
    for id in verified_answers_ids:
        answer = db.uk_assessed_statements.find_one({'_id': id})
...
statements = db.uk_assessed_0523.find({"verified": True, "related_entities": {'$exists': True}})
...
for statement in statements:
    ...
    for entity in related_entities:
        print(entity)
        if conflict_free(verified_answers_ids, entity):
            for id in verified_answers_ids:
                answer = db.uk_assessed_statements.find_one({'_id': id})
                metric = answer['name'].split('+')
                try:
                    ...
                    try:
                        ...
                        comments = ''
                    copied_answer = api.add_research_metric_answer(
