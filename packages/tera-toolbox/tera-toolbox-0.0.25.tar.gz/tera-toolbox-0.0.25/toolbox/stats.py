...
def total_return(returns):
    """
    Calculates the total compounded return for an asset.

    Args:
        returns (pd.Series or pd.DataFrame): The input returns series for the asset.

    Returns:
        float: The total compounded return for the asset.
    """
    if not isinstance(returns, (pd.Series, pd.DataFrame)):
        raise ValueError("Input must be a pandas Series or DataFrame.")

    # Calculate total compounded return
    compounded_return = returns.add(1).prod() - 1
...
def cumulative_return(returns, aggregate=None, compounded=True, periods=252):
    """
    ...
    Args:
     ...
     Returns:
        float: The calculated cumulative return over the period.
    """
    if not isinstance(returns, (pd.Series, pd.DataFrame)):
        raise ValueError("Input must be a pandas Series or DataFrame.")
    ...
    return (1 + total) ** (periods / returns.count()) - 1


def rolling_returns(data, window=252, periods_per_year=252):
    """
    Calculate rolling window returns for a given window size.

    Args:
        data (pd.Series or pd.DataFrame): The input data series.
    ...
    roll_returns = data.rolling(window).apply(
        lambda x: cumulative_return(x, periods=periods_per_year)
    )

    return roll_returns[window:]
...
def rolling_volatility(returns, window=252, periods_per_year=252, annualize=True):
    """
    ...
    # Calculate rolling volatility using the volatility function
    roll_volatility = returns.rolling(window).apply(
        lambda x: volatility(x, periods=periods_per_year, annualize=annualize),
        raw=True,
    )

    return roll_volatility[window:]
...
def rolling_sharpe(returns, rf, window=252, periods_per_year=252, annualize=True):
    """
    ...
    # Apply the sharpe function to each rolling window of the excess returns
    rolling_sharpe = excess_returns.rolling(window).apply(
        lambda x: sharpe(x, periods=periods_per_year, annualize=annualize),
        raw=True,
    )

    return rolling_sharpe[window:]
...
def sortino(returns, rf, periods=252, annualize=True, smart=False):
    ...
    downside = np.sqrt(
        (excess_returns[excess_returns < 0] ** 2).sum() / len(excess_returns)
    )
    if smart:
...
def rolling_sortino(
    returns, rf, rolling_period=126, annualize=True, periods_per_year=252
):
    excess_returns = utils.to_excess_returns(returns, rf, periods_per_year)
    downside = (
        excess_returns.rolling(rolling_period).apply(
