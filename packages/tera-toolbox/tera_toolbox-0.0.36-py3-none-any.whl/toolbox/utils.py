...
def rebase(prices, base=100, base_date=None, matching=False):
    ...
    if matching:
        ...
        latest_base_date = max([prices[column].first_valid_index() for column in prices.columns])
    ...
    for column in prices.columns:
        ...
        actual_base_date = actual_base_date if not base_date else max(actual_base_date, base_date)
...
def group_returns(returns, groupby, compounded=True, periods=None, annualize=False):
    ...
    """
    return returns.groupby(groupby).apply(stats.total_return, compounded=compounded, periods=periods, annualize=annualize)
...
def drawdown_details(drawdown):
    ...
    def _drawdown_details(single_drawdown):
        ...
        for start, end in zip(starts, ends):
            ...
            data.append([start.date(), valley_date.date(), end.date(), (end - start).days, max_drawdown * 100])
    ...
    if isinstance(drawdown, pd.DataFrame):
        return pd.concat({col: _drawdown_details(drawdown[col]) for col in drawdown.columns}, axis=1)
