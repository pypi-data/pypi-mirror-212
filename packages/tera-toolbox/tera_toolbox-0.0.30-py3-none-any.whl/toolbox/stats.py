...
def rolling_returns(data, window=252, periods_per_year=252):
    ...
    roll_returns = data.rolling(window).apply(
        lambda x: total_return(x, periods=periods_per_year, annualize=True)
    )
...
def rolling_sortino(
    returns, rf, window=252, periods_per_year=252, annualize=True, target=0
):
    """
    ...
    # Apply the sortino function to each rolling window of the excess returns
    rolling_sortino = excess_returns.rolling(window).apply(
        lambda x: sortino(
            x, periods=periods_per_year, annualize=annualize, target=target
        ),
        raw=True,
    )

    return rolling_sortino[window:]
