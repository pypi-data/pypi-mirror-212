...
async def recv_json(reader, timeout=None):
    ...
    try:
        ...
        coroutine = reader.readline()

        if timeout is None:
            data = await coroutine
        else:
            data = await asyncio.wait_for(coroutine, timeout)
...
class UnixSocketClient:
    ...
    async def recv_json(self, timeout=None):
        ...
        return await recv_json(self._reader, timeout)

    async def send_json(self, data):

        return await send_json(self._writer, data)
