...
from typing import Iterator, Union, Literal, List, Tuple
...
class MySql:
    ...
    def insert(self, items: Union[dict, list, tuple], values: list = None, ignore: bool = False) -> None:
        ...
        if isinstance(items, (list, tuple)):
            self.insert_batch(items, ignore)
        else:
            sql = self.sql_base.insert(items, values, ignore=ignore)
            try:
                self.execute(sql, False, mode="insert")
    ...
    def insert_batch(self, items: List[Tuple[dict, Union[dict, str]]], ignore: bool = True) -> None:
        """
        批量插入 只是把之前的单条插入给分出来了而已
        """
        self._check_table()
        sql = self.sql_base.insert(items, ignore=ignore)
        try:
            self.execute(sql, True, mode="insert")
        except DuplicateEntryException:
            for each_key in items:
                new_sql = self.sql_base.insert(each_key, ignore=ignore)
                self.execute(new_sql, log=False, mode="insert")
    ...
    def update_batch(self, items: List[Tuple[dict, Union[dict, str]]]) -> None:
        """
