...
def _walrus_wrapper_lambda_3d333dc7cc574f90aeebdbcc93d999df( NAME, DOCS, FLAG, ENUM, MISS, MODL):
    """Wrapper function for lambda definitions."""
    if False:
        name = test = NotImplemented

    def _walrus_wrapper_name_f2a2b9d814884089ae1ca9ab3ab0212c(expr):
        """Wrapper function for assignment expression."""
        nonlocal name
        name = expr
        return name

    def _walrus_wrapper_test_d10a70b68c2b490ea9ce839d0c48b982(expr):
        """Wrapper function for assignment expression."""
        nonlocal test
        test = expr
        return test
...
"""{(_walrus_wrapper_name_f2a2b9d814884089ae1ca9ab3ab0212c(DOCS.split(' [', maxsplit=1)[0]))}
{'=' * (len(name) + 6)}
...
class {NAME}(IntEnum):
    ...
    def _missing_(cls, value: 'int') -> '{NAME}':
        ...
        {MISS}
        {'' if (_walrus_wrapper_test_d10a70b68c2b490ea9ce839d0c48b982(''.join(MISS.splitlines()[-1:]))).startswith('return') or test[8:].startswith('return') else 'return super()._missing_(value)'}
'''.strip()

#: Default constant template of enumerate registry from IANA CSV.
LINE =_walrus_wrapper_lambda_3d333dc7cc574f90aeebdbcc93d999df  # type: Callable[[str, str, str, str, str, str], str]
