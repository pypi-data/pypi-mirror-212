...
class Protocol(Generic[PT, ST], metaclass=abc.ABCMeta):
    ...
    def expand_comp(value: 'str | Protocol | Type[Protocol]') -> 'tuple':
        ...
        if isinstance(value, type) and issubclass(value, Protocol):
            ...
            if False:
                proto = NotImplemented

            def _walrus_wrapper_proto_89e7c5addda143c7a9e3f250086257dc(expr):
                """Wrapper function for assignment expression."""
                nonlocal proto
                proto = expr
                return proto
            if (_walrus_wrapper_proto_89e7c5addda143c7a9e3f250086257dc(protocols_registry.get(value.upper()))) is not None:
    ...
    def __repr__(self) -> 'str':
        ...
        if False:
            cached = NotImplemented

        def _walrus_wrapper_cached_8e1670df752d4f8ab58fa7c1bc2a1f84(expr):
            """Wrapper function for assignment expression."""
            nonlocal cached
            cached = expr
            return cached

        if (_walrus_wrapper_cached_8e1670df752d4f8ab58fa7c1bc2a1f84(self.__cached__.get('__repr__'))) is not None:
            return cached
    ...
    def __str__(self) -> 'str':
        ...
        def _walrus_wrapper_cached_600665b57a3647c9a8c3e85b8402903b(expr):
            """Wrapper function for assignment expression."""
            nonlocal cached
            cached = expr
            return cached

        if (_walrus_wrapper_cached_600665b57a3647c9a8c3e85b8402903b(self.__cached__.get('__str__'))) is not None:
            return cached
    ...
    def __len__(self) -> 'int':
        ...
        def _walrus_wrapper_cached_cde697d9da83405caf216c4dce363a80(expr):
            """Wrapper function for assignment expression."""
            nonlocal cached
            cached = expr
            return cached

        if (_walrus_wrapper_cached_cde697d9da83405caf216c4dce363a80(self.__cached__.get('__len__'))) is not None:
            return cached
    ...
    def _check_term_threshold(self) -> bool:
        ...
        def _walrus_wrapper_layer_63bbbca0418543b88951de8f4adf1df4(expr):
            """Wrapper function for assignment expression."""
            nonlocal layer
            layer = expr
            return layer

        if self._exlayer is None or (_walrus_wrapper_layer_63bbbca0418543b88951de8f4adf1df4(self.__layer__)) is None:
            layer_match = False
        else:
