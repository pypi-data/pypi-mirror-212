...
def _walrus_wrapper_lambda_8f38f38955324a8a97e61913efdc7a6e( NAME, DOCS, FLAG, ENUM, MISS, MODL):
    """Wrapper function for lambda definitions."""
    if False:
        name = test = NotImplemented

    def _walrus_wrapper_name_eb0a981695f445998d5188615e3595a8(expr):
        """Wrapper function for assignment expression."""
        nonlocal name
        name = expr
        return name

    def _walrus_wrapper_test_25047055a5ea47ecbe5e63a7624ff289(expr):
        """Wrapper function for assignment expression."""
        nonlocal test
        test = expr
        return test
...
"""{(_walrus_wrapper_name_eb0a981695f445998d5188615e3595a8(DOCS.split(' [', maxsplit=1)[0]))}
{'=' * (len(name) + 6)}
...
class {NAME}(IntEnum):
    ...
    def _missing_(cls, value: 'int') -> '{NAME}':
        ...
        {MISS}
        {'' if (_walrus_wrapper_test_25047055a5ea47ecbe5e63a7624ff289(''.join(MISS.splitlines()[-1:]))).startswith('return') or test[8:].startswith('return') else 'return super()._missing_(value)'}
'''.strip()

#: Default constant template of enumerate registry from IANA CSV.
LINE =_walrus_wrapper_lambda_8f38f38955324a8a97e61913efdc7a6e  # type: Callable[[str, str, str, str, str, str], str]
